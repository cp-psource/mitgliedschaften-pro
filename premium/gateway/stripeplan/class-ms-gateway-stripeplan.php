...
class MS_Gateway_Stripeplan extends MS_Gateway {
	...
	public function after_load() {
		...
		$this->id 							= self::ID;
		$this->name 						= __( 'Stripe Abonnements Gateway', 'membership2' );
		$this->group 						= 'Stripe';
														...
														if ( $current_date != $subscription->start_date ) {
															...
															if ( $invoice ) {
																...
																if ( 0 == $invoice->total ) {
																	...
																	$invoice->changed();
																	$success = true;
																	$notes = __( 'Für die kostenlose Mitgliedschaft ist keine Zahlung erforderlich', 'membership2' );
																...
																$this->log( 'Rechnung nicht gefunden' );
															}
														} else {
															$this->log( 'Die Rechnung wurde bereits seit dem ersten Datum bezahlt' );
														}
														
													break;
													case 'customer.subscription.deleted' :
													case 'invoice.payment_failed' :
														$notes .= __( 'Mitgliedschaft gekündigt oder Zahlung per Webhook fehlgeschlagen', 'membership2' );
														$success = false;
														$member->cancel_membership( $membership->id );
														$member->save();
													break;
													default : 
														$notes = sprintf( __( 'Stripe Webhook "%s" empfangen', 'membership2' ), $event->type );
													break;
												}
											} else {
												$this->log( 'Ungültiges stripe Sub' );
											}
										}
		...
		if ( ! empty( $_POST['stripeToken'] ) ) {
			...
			try {
				...
				if ( 0 == $invoice->total ) {
					...
					$invoice->changed();
					$success = true;
					$note = __( 'Für die kostenlose Mitgliedschaft ist keine Zahlung erforderlich', 'membership2' );
				} else {
