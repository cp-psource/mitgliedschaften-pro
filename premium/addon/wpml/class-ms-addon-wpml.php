...
class MS_Addon_Wpml extends MS_Addon {
	...
	public function register( $list ) {
		$list[ self::ID ] = (object) array(
			...
			'description' => __( 'Verwende WPML, um Plugin-Nachrichten zu übersetzen.', 'membership2' ),
		);

		if ( ! self::wpml_active() ) {
			$list[ self::ID ]->description .= sprintf(
				'<br /><b>%s</b>',
				__( 'Aktiviere WPML, um dieses Add-On zu verwenden', 'membership2' )
			);
			$list[ self::ID ]->action = '-';
		}
	...
	protected function check_requirements() {
		if ( ! self::wpml_active() ) {
			mslib3()->ui->admin_message(
				sprintf(
					'<b>%s</b><br>%s',
					__( 'WPML nicht aktiv!', 'membership2' ),
					__( 'Kopf hoch: Aktiviere das WPML-Plugin, um alle Übersetzungsfunktionen von Mitgliedschaften zu aktivieren.', 'membership2' )
				),
				'err'
			);
			return;
		...
		if ( version_compare( ICL_SITEPRESS_VERSION, '3.2', 'lt' ) ) {
			mslib3()->ui->admin_message(
				sprintf(
					'<b>%s</b><br>%s',
					__( 'Großartig, Du verwendest WPML!', 'membership2' ),
					__( 'Kopf hoch: Deine Version von WPML ist veraltet. Bitte aktualisiere WPML auf Version <b>3.2 oder höher</b>, um alle Übersetzungsfunktionen von Mitgliedschaften zu aktivieren.', 'membership2' )
				),
				'err'
			);
			return;
