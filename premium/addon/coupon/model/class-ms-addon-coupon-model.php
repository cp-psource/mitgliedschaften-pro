...
class MS_Addon_Coupon_Model extends MS_Model_CustomPostType {
	...
	public static function get_register_post_type_args() {
		$args = array(
			'label' => __( 'Mitgliedschaften Gutscheine', 'membership2' ),
	...
	public static function get_discount_duration() {
		...
		if ( empty( $durations ) ) {
			$durations = array(
				self::DURATION_ONCE => __( 'die erste Rechnung', 'membership2' ),
				self::DURATION_ALWAYS => __( 'auf alle Rechnungen', 'membership2' ),
			);
		}
	...
	public function is_valid( $membership_id = 0, $invoice = null ) {
		...
		if ( empty( $this->code ) ) {
			$this->coupon_message = __( 'Gutscheincode nicht gefunden.', 'membership2' );
			$valid = false;
		} elseif ( $this->max_uses && $this->used >= $this->max_uses ) {
			$this->coupon_message = __( 'Für diesen Code sind keine Gutscheine mehr vorhanden.', 'membership2' );
			$valid = false;
		} elseif ( ! empty( $this->start_date ) && $start_date_timestamp > $timestamp ) {
			$this->coupon_message = __( 'Dieser Gutschein ist noch nicht gültig.', 'membership2' );
			$valid = false;
		} elseif ( ! empty( $this->expire_date ) && $expire_date_timestamp < $timestamp ) {
			$this->coupon_message = __( 'Dieser Gutschein ist abgelaufen.', 'membership2' );
			$valid = false;
		} elseif ( MS_Addon_Coupon_Model::DURATION_ONCE === $this->duration && ! is_null( $invoice ) ) {
			if ( $invoice->is_valid() && 1 < $invoice->invoice_number ) {
				$this->coupon_message = __( 'Dieser Gutschein kann nur bei der ersten Zahlung verwendet werden.', 'membership2' );
				$valid = false;
			}
		} else {
			...
			if ( ! $membership_allowed ) {
				$this->coupon_message = __( 'Dieser Gutschein gilt nicht für diese Mitgliedschaft.', 'membership2' );
				$valid = false;
			}
		}
	...
	public function get_discount_value( $subscription ) {
		...
		if ( $this->is_valid( $membership->id, $subscription->get_current_invoice( false ) ) ) {
			...
			$this->coupon_message = sprintf(
				__( 'Gutschein angewendet: %1$s', 'membership2' ),
				$this->code
			);
		}
		...
		switch ( $property ) {
			...
			case 'remaining_uses':
				if ( $this->max_uses > 0 ) {
					...
					$value = __( 'Unbegrenzt', 'membership2' );
				}
				break;
