...
class MS_Addon_Invitation_Model extends MS_Model_CustomPostType {
	...
	public static function load_by_code( $code ) {
		...
		if ( ! $model->is_valid() ) {
			$model->invitation_message = __( 'Einladungscode nicht gefunden.', 'membership2' );
		}
	...
	public static function get_application( $user_id, $membership_id ) {
		...
		if ( $invitation->is_valid() ) {
			$invitation->invitation_message = __( 'Der Einladungscode ist korrekt.', 'membership2' );
		}
	...
	public function is_valid( $membership_id = 0 ) {
		...
		if ( $this->max_uses && $this->used >= $this->max_uses ) {
			$this->invitation_message = __( 'Dieser Einladungscode ist nicht mehr gültig.', 'membership2' );
			$valid = false;
		} elseif ( ! empty( $this->start_date ) && strtotime( $this->start_date ) > $timestamp ) {
			$this->invitation_message = __( 'Diese Einladung ist noch nicht gültig.', 'membership2' );
			$valid = false;
		} elseif ( ! empty( $this->expire_date ) && strtotime( $this->expire_date ) < $timestamp ) {
			$this->invitation_message = __( 'Diese Einladung ist abgelaufen.', 'membership2' );
			$valid = false;
		/*} elseif ( ! $this->check_invitation_user_usage() ) {
			...
			if ( ! $membership_allowed ) {
				$this->invitation_message = __( 'Diese Einladung gilt nicht für diese Mitgliedschaft.', 'membership2' );
				$valid = false;
			}else{
