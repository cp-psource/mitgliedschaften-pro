...
class MS_Addon_Mailchimp extends MS_Addon {
	...
	public function register( $list ) {
		$list[ self::ID ] = (object) array(
			...
			'description' 	=> __( 'Aktiviere die MailChimp-Integration.', 'membership2' ),
	...
	private static function mailchimp_log( $message ) {
		if( defined( 'WP_DEBUG' ) && WP_DEBUG === true ) { 
			mslib3()->debug->log( '[M2] Mailchimp Fehler : ' . $message );
		}
	}
	...
	public function mc_fields_for_ms( $fields, $membership, $data ) {
		...
		$mail_list = self::get_mail_lists( __( 'Standard', 'membership2' ) );

		$fields['ms_mc'] = array(
			'id' 			=> 'ms_mc',
			'type' 			=> MS_Helper_Html::INPUT_TYPE_SELECT,
			'title' 		=> __( 'Mailchimp Liste', 'membership2' ),
			'desc' 			=> __( 'Du kannst eine Liste für diese Mitgliedschaft auswählen.', 'membership2' ),
			'class' 		=> 'ms-mc',
			'before' 		=> __( 'Wähle eine Liste aus', 'membership2' ),
