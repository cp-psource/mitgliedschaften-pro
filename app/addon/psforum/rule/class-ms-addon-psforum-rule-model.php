 ...
 * @subpackage Model
 */
class MS_Addon_Psforum_Rule_Model extends MS_Rule {

	/**
	 * Custom Post Type names that are used by PSForum
	 *
	 * @since  1.0.0
	 */
	 ...
	 */
	protected $rule_type = MS_Addon_PsForum_Rule::RULE_ID;
	...
	public function has_access( $id, $admin_has_access = true ) {
				...
				if ( MS_Addon_Psforum::is_active() ) {
					switch ( $post_type ) {
						...
						case self::CPT_BB_TOPIC:
							if ( function_exists( 'psf_get_topic_forum_id' ) ) {
								$forum_id 	= psf_get_topic_forum_id( $id );
								$has_access = parent::has_access( $forum_id, $admin_has_access );
							}
							break;

						case self::CPT_BB_REPLY:
							if ( function_exists( 'psf_get_reply_forum_id' ) ) {
								$forum_id 	= psf_get_reply_forum_id( $id );
								$has_access = parent::has_access( $forum_id, $admin_has_access );
							}
							break;
					}
		...
		return apply_filters(
			'ms_addon_psforum_model_rule_has_access',
			$has_access,
			$id,
			$this
		);
	...
	/**
	 * Adds filter for posts query to remove all protected psforum custom post types.
	 *
	...
	public function protect_posts( $wp_query ) {
		do_action(
			'ms_addon_psforum_model_rule_protect_posts_before',
			$wp_query,
			$this
		);
		...
		do_action(
			'ms_addon_psforum_model_rule_protect_posts_after',
			$wp_query,
			$this
		);
	}
	...
	private function get_current_post_id() {
		...
		return apply_filters(
			'ms_addon_psforum_model_rule_get_current_post_id',
			$post_id,
			$this
		);
	}
	...
	public function get_content_count( $args = null ) {
		...
		return apply_filters(
			'ms_addon_psforum_model_rule_get_content_count',
			$count
		);
	}
	...
	public function get_contents( $args = null ) {
		...
		return apply_filters(
			'ms_addon_psforum_model_rule_get_contents',
			$contents
		);
	}
	...
	public function get_query_args( $args = null ) {
		...
		return apply_filters(
			'ms_addon_psforum_model_rule_get_query_args',
			$args
		);
	}

	/**
	 * Get PS Forum custom post types.
	 *
	 * @since  1.0.0
	 *
	 * @return array The psforum custom post types.
	 */
	public static function get_bb_cpt() {
		return apply_filters(
			'ms_addon_psforum_rule_model_get_bb_cpt',
			array(
				self::CPT_BB_FORUM,
				self::CPT_BB_TOPIC,
				self::CPT_BB_REPLY,
