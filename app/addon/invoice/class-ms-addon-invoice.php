...
class MS_Addon_Invoice extends MS_Addon {
	...
	public static function sequence_types() {
		return apply_filters( 'ms_addon_invoice_sequence_types', array(
			self::DEFAULT_SEQUENCE 		=> __( 'Grundlegende Generierung der Rechnungs-ID (Standard)', 'membership2' ),
			self::PROGRESSIVE_SEQUENCE 	=> __( 'Progressive Generierung der Rechnungs-ID', 'membership2' ),
			self::CUSTOM_SEQUENCE 		=> __( 'Benutzerdefinierte Rechnungs-ID-Generierung für alle Gateways', 'membership2' ),
			self::COMBINED				=> __( 'Kombinierte Generierung, die benutzerdefinierte Präfix- und progressive Rechnungs-ID-Einstellungen kombiniert', 'membership2' ),
		) );
	}
	...
	public function register( $list ) {
		...
		$list[ self::ID ] = (object) array(
			'name' 			=> __( 'Zusätzliche Rechnungseinstellungen', 'membership2' ),
			'description' 	=> __( 'Mehr Kontrolle über Deine Rechnungen', 'membership2' ),
			'icon' 			=> 'wpmui-fa wpmui-fa-credit-card',
			'footer' 		=> sprintf( '<i class="dashicons dashicons dashicons-admin-settings"></i> %s', __( 'Optionen verfügbar', 'membership2' ) ),
			'class' 		=> 'ms-options',
			'details' 		=> array(
				array(
					'id' 			=> 'invoice_sequence_type',
					'type' 			=> MS_Helper_Html::INPUT_TYPE_SELECT,
					'title' 		=> __( 'Wähle die Rechnungsnummernfolge', 'membership2' ),
	...
	public function render_sequence_type_default( $settings, $total_invoices ) {
		return __( "Rechnungs-IDs werden in der Standardreihenfolge generiert", "membership2" );
	}
		...
		if ( $total_invoices > 0 && !$invoices_set ) {
			...
			<div class="ms-common-prefix">
				<?php MS_Helper_Html::html_element( array(
					...
					'desc' 		=> sprintf( __( "Aktualisiere die IDs der aktuellen (%d) Rechnungen.", 'membership2' ), $total_invoices ),
					'value' 	=> __( 'Aktualisieren', 'membership2' ),
				...
				MS_Helper_Html::html_element( array(
					'type' 	=> MS_Helper_Html::TYPE_HTML_TEXT,
					'value' => __( "Rechnungs-IDs werden in progressiver Reihenfolge generiert. Dies hängt von der Gesamtzahl der Rechnungen ab, die Du in Deiner Installation hast", "membership2" )
				) ); ?>
			...
			return __( "Rechnungs-IDs werden in progressiver Reihenfolge generiert. Dies hängt von der Gesamtzahl der Rechnungen ab, die Du in Deiner Installation hast", "membership2" );
		}
	}
	...
	public function render_sequence_type_custom( $settings, $total_invoices ) {
		...
		<div class="ms-common-prefix">
			<?php MS_Helper_Html::html_element( array(
				...
				'desc' 	=> __( 'Rechnungspräfix für alle Rechnungen', 'membership2' ),
	...
	public function render_sequence_type_combined( $settings, $total_invoices ) {
		...
		<div class="ms-common-prefix">
			<?php MS_Helper_Html::html_element( array(
				'id' 	=> 'invoice_prefix',
				'type' 	=> MS_Helper_Html::INPUT_TYPE_TEXT,
				'desc' 	=> __( 'Rechnungspräfix für alle Rechnungen. Dies wird mit der progressiven Sequenz kombiniert', 'membership2' ),
				'value' => $settings->invoice['invoice_prefix'],
				'class' => 'ms-text-large',
				'data_ms' => array(
					'field' 	=> 'invoice_prefix',
