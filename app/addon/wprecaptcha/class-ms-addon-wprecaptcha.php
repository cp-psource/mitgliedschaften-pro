...
class MS_Addon_Wprecaptcha extends MS_Addon {
	...
	public function register( $list ) {
		$list[ self::ID ] = (object) array(
			...
			'description' 	=> __( 'Aktiviere die WP reCaptcha-Integration (inaktiv).', 'membership2' ),
		...
		if ( ! self::wp_recaptcha_active() ) {
			$list[ self::ID ]->description .= sprintf(
				'<br /><b>%s</b>',
				__( 'Aktiviere WP reCaptcha, um dieses Add-On zu verwenden', 'membership2' )
			);
			$list[ self::ID ]->action = '-';
		} else {
			...
			$list[ self::ID ]->description = sprintf(
				'<b>%s</b>',
				__( 'WP reCaptcha aktiviert', 'membership2' )
			);
		}
	...
	public function check_captcha_validation( $errors ) {
		...
		if ( ! $valid ) {
			$errors->add( 'captcha_error', __( 'Ungültiges Captcha', 'membership2' ) );
		} 
