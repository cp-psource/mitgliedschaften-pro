...
class MS_Addon_Taxamo_Api extends MS_Controller {
	...
	static public function tax_info( $amount = 0 ) {
		...
		if ( null === $Info ) {
			...
			try {
				...
				MS_Helper_Debug::debug_log( 'Taxamo Fehler: ' . $ex->getMessage() );
			}
		}

		if ( ! is_object( $Info ) ) { $Info = (object) array(); }
		if ( ! isset( $Info->name ) ) { $Info->name = __( 'Keine Steuer', 'membership2' ); }
	...
	static public function get_country_codes( $type = 'prefix' ) {
		if ( null === self::$Countries ) {
			...
			self::$Countries['XX'] = '- ' . __( 'Außerhalb der EU', 'membership2' ) . ' -';
			self::$Countries_Prefix['XX'] = '- ' . __( 'Außerhalb der EU', 'membership2' ) . ' -';
		}
