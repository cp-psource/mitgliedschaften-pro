...
class MS_Model_Import extends MS_Model {
	...
	public function import_member( $obj ) {
		...
		if ( $wpuser ) {
			...
			if ( is_numeric( $wpuser ) ) {
				...
				$this->errors[] = sprintf(
					__( 'Mitglied <strong>%1$s</strong> konnte nicht importiert werden (%2$s)', 'membership2' ),
	...
	protected function import_subscription( $member, $obj ) {
		...
		if ( empty( $membership ) ) {
			$this->errors[] = sprintf(
				__( 'Eine Mitgliedschaft für Benutzer <strong>%1$s</strong> (%2$s) konnte nicht importiert werden.', 'membership2' ),
		...
		if ( $membership->is_base() ) {
			$this->errors[] = sprintf(
				__( 'Die Basismitgliedschaft %2$s für <strong>%1$s</strong> wurde nicht importiert', 'membership2' ),
				esc_attr( $member->username ),
