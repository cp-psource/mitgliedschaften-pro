...
class MS_Model_Upgrade extends MS_Model {
	...
	public static function update( $force = false, $rewrite = false ) {
		...
		if ( $force || $version_changed ) {
			...
			if ( ! $version_changed ) {
				$msg[] = sprintf(
					__( '<strong>Mitgliedschaften</strong> ist für Version %1$s eingerichtet!' , 'membership2' ),
					$new_version
				);
			} else {
				$msg[] = sprintf(
					__( '<strong>Mitgliedschaften</strong> wurde auf Version %1$s aktualisiert!' , 'membership2' ),
					$new_version
				);
			}
			...
			if ( ! empty( $new_pages ) ) {
				$msg[] = sprintf(
					__( 'Neue Mitgliederseiten erstellt: "%1$s".', 'membership2' ),
					implode( '", "', $new_pages )
				);
			}
	...
	static private function check_settings() {
		...
		if ( ! $Setting_Check_Done ) {
			...
			if ( is_multisite() ) {
				...
				if ( MS_Plugin::is_network_wide() ) {
					...
					if ( ! is_plugin_active_for_network( MS_PLUGIN ) ) {
						...
						mslib3()->ui->admin_message(
							__( 'Info: Mitgliedschaften ist nicht netzwerkweit aktiviert', 'membership2' )
						);
					}
				}
			}
			...
			if ( false === ( get_option('permalink_structure') ) ) {
				mslib3()->ui->admin_message(
					sprintf(
						__( 'Du musst %sPretty Permalinks%s in Deiner %sPermalink-Struktur%s aktivieren, um sicherzustellen, dass Mitgliedschaften ordnungsgemäß funktioniert.', 'membership2' ),
						'<strong>',
						'</strong>',
						'<a href="' . admin_url( 'options-permalink.php' ) . '">',
						'</a>'
	...
	static public function maybe_reset() {
		...
		if ( ! $Reset_Done ) {
			...
			$msg = __( 'Mitgliedschaften erfolgreich zurückgesetzt!', 'membership2' );
	...
	static public function maybe_fix_stripe_subs() {
	    ...
	    if ( ! $Fix_Done ) {
            ...
            self::fix_subs();
            $msg = __( 'Mitgliedschaften Abonnements behoben!', 'membership2' );
            mslib3()->ui->admin_message( $msg );
	...
	static private function maybe_restore() {
		...
		if ( ! $Restore_Done ) {
			...
			if ( mslib3()->updates->restore( $snapshot ) ) {
				printf(
					'<p>' .
					__( 'Der Mitgliedschaften-Snapshot "%s" wurde wiederhergestellt!', 'membership2' ) .
					'</p>',
					$snapshot
				);
				...
				printf(
					'<p><b>' .
					__( 'Um zu verhindern, dass die Datenbank erneut automatisch aktualisiert wird, hören wir hier auf!', 'membership2' ) .
					'</b></p>'
				);

				printf(
					'<p>' .
					__( 'Sie haben jetzt die Möglichkeit, <br />(A) das Plugin über FTP auf eine frühere Version herunterzustufen oder <br />(B), um %sden Aktualisierungsprozess%s erneut auszuführen.', 'membership2' ) .
					'</p>',
					'<a href="' . MS_Controller_Plugin::get_admin_url( 'MENU_SLUG' ) . '">',
					'</a>'
				);

				wp_die( '', 'Schnappschuss wiederhergestellt' );
			}
		}
	}
