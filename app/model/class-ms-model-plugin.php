...
class MS_Model_Plugin extends MS_Model {
	...
	public function __construct() {
		...
		if ( class_exists( 'M_Membership' ) ) {
			mslib3()->ui->admin_message(
				__( '<b>Warnung</b>: Die alte Version des Mitgliedschafts-Plugins ist aktiv und führt zu Konflikten mit dem neuen Mitgliedschaften-Plugin. Bitte deaktiviere das alte Mitgliedschafts-Plugin.', 'membership2' ),
				'red'
			);
		}
	...
	public function get_access_info() {
		...
		if ( null === $Info ) {
			...
			if ( $this->member->is_normal_admin() ) {
				...
				if ( $simulation ) {
					$Info['reason'][] = __( 'Zulassen: Admin-User hat immer Zugriff', 'membership2' );
				}
				...
				if ( $ms_page ) {
					...
					if ( $simulation ) {
						$Info['reason'][] = __( 'Zulassen: Dies ist eine Mitgliederseite', 'membership2' );
					}
				}
				...
				foreach ( $this->member->subscriptions as $subscription ) {
					...
					if ( ! $this->member->has_membership( $subscription->membership_id ) ) {
						if ( $simulation ) {
							$Info['reason'][] = sprintf(
								__( 'Übersprungen: Kein Mitglied von "%s"', 'membership2' ),
					...
					if ( true !== $Info['has_access'] ) {
						...
						if ( null === $access ) {
							if ( $simulation ) {
								$Info['reason'][] = sprintf(
									__( 'Ignoriert: Mitgliedschaft "%s"', 'membership2' ),
									$membership->name
								);
						...
						if ( $simulation ) {
							$Info['reason'][] 				= sprintf(
								__( '%s: Mitgliedschaft "%s"', 'membership2' ),
								$access ? __( 'Zulassen', 'membership2' ) : __( 'Verweigern', 'membership2' ),
								$membership->name
							);
				...
				if ( null === $Info['has_access'] ) {
					...
					if ( $simulation ) {
						$Info['reason'][] 					= __( 'Zulassen: Seite ist nicht geschützt', 'membership2' );
					}
				}
			...
			if ( $simulation ) {
				...
				if ( WP_DEBUG && isset( $_GET['explain'] ) && 'access' == $_GET['explain'] ) {
					...
					echo '<h3>Note</h3>';
					echo '<p>Um den URL-Parameter <code>?explain=access</code> zu deaktivieren, musst Du <code>WP_DEBUG</code> auf false festlegen.</p>';
					echo '<hr><h3>Recent Access checks</h3>';

					mslib3()->debug->stacktrace_off();
					foreach ( $access as $item ) {
						if ( $item['has_access'] ) {
							$label = __( 'Zulassen', 'membership2' );
						} else {
							$label = __( 'Verweigern', 'membership2' );
						}
						printf(
	...
	public function setup_rules() {
		...
		if ( ! did_action( 'ms_init_done' ) ) {
			throw new Exception( 'setup_rules() wird zu früh aufgerufen.', 1 );
			return;
		}
	...
	public function setup_protection() {
		...
		// Make sure we stick to the correct workflow.
		if ( ! did_action( 'ms_init_done' ) ) {
			throw new Exception( 'setup_protection() wird zu früh aufgerufen.', 1 );
			return;
		}
	...
	public function setup_admin_protection() {
		...
		// Make sure we stick to the correct workflow.
		if ( ! did_action( 'ms_init_done' ) ) {
			throw new Exception( 'setup_admin_protection() wird zu früh aufgerufen.', 1 );
			return;
		}
	...
	public function cron_time_period( $periods ) {
		...
		$periods['6hours'] = array(
			...
			'display' 	=> __( 'Alle 6 Stunden', 'membership2' ),
		);
		$periods['30mins'] = array(
			'interval' 	=> 30 * MINUTE_IN_SECONDS,
			'display' 	=> __( 'Alle 30 Minuten', 'membership2' ),
		);
