...
class MS_Model_Communication extends MS_Model_CustomPostType {
	...
	public static function get_register_post_type_args() {
		$args = array(
			'label' 				=> __( 'Mitgliedschaft E-Mail-Vorlagen', 'membership2' ),
	...
	public static function get_communication_type_titles( $membership = null ) {
		$type_titles = array(
			self::COMM_TYPE_SIGNUP 					=> __( 'Anmelden - Benutzerkonto erstellt', 'membership2' ),
			self::COMM_TYPE_RESETPASSWORD 			=> __( 'Anmeldung - Passwort vergessen', 'membership2' ),
			self::COMM_TYPE_REGISTRATION 			=> __( 'Abonnement - Mit Zahlung abgeschlossen', 'membership2' ),
			self::COMM_TYPE_REGISTRATION_FREE 		=> __( 'Abonnement - Abgeschlossen (kostenlose Mitgliedschaft)', 'membership2' ),
			self::COMM_TYPE_REGISTRATION_VERIFY 	=> __( 'Anmeldung - Überprüfe Deine E-Mail', 'membership2' ),
			self::COMM_TYPE_RENEWED 				=> __( 'Abonnement - erneuert', 'membership2' ),
			self::COMM_TYPE_BEFORE_FINISHES 		=> __( 'Abonnement - Vor Ablauf', 'membership2' ),
			self::COMM_TYPE_FINISHED 				=> __( 'Abonnement - Abgelaufen', 'membership2' ),
			self::COMM_TYPE_AFTER_FINISHES 			=> __( 'Abonnement - Nach Ablauf', 'membership2' ),
			self::COMM_TYPE_CANCELLED 				=> __( 'Abonnement - Kündigt', 'membership2' ),
			self::COMM_TYPE_BEFORE_TRIAL_FINISHES 	=> __( 'Abonnement - Testversion beendet', 'membership2' ),
			self::COMM_TYPE_INFO_UPDATE 			=> __( 'Zahlung - Profil aktualisiert', 'membership2' ),
			self::COMM_TYPE_CREDIT_CARD_EXPIRE 		=> __( 'Zahlung - Kreditkarte läuft ab', 'membership2' ),
			self::COMM_TYPE_INVOICE 				=> __( 'Zahlung - Quittung/Rechnung', 'membership2' ),
			self::COMM_TYPE_FAILED_PAYMENT 			=> __( 'Zahlung fehlgeschlagen', 'membership2' ),
			self::COMM_TYPE_BEFORE_PAYMENT_DUE 		=> __( 'Zahlung - Vor Fälligkeit', 'membership2' ),
			self::COMM_TYPE_AFTER_PAYMENT_DUE 		=> __( 'Zahlung - Nach Fälligkeit', 'membership2' ),
		);
	...
	public function __construct() {
		$this->comm_vars = array(
			self::COMM_VAR_MS_NAME 					=> __( 'Abonnement: Name der Mitgliedschaft', 'membership2' ),
			self::COMM_VAR_MS_DESCRIPTION 			=> __( 'Abonnement: Beschreibung der Mitgliedschaft', 'membership2' ),
			self::COMM_VAR_MS_REMAINING_DAYS 		=> __( 'Abonnement: Verbleibende Tage', 'membership2' ),
			self::COMM_VAR_MS_REMAINING_TRIAL_DAYS 	=> __( 'Abonnement: Verbleibende Testtage', 'membership2' ),
			self::COMM_VAR_MS_EXPIRY_DATE 			=> __( 'Abonnement: Ablaufdatum', 'membership2' ),
			self::COMM_VAR_MS_INVOICE 				=> __( 'Abonnement: Aktuelle Rechnung', 'membership2' ),
			self::COMM_VAR_USER_DISPLAY_NAME 		=> __( 'Benutzer: Anzeigename', 'membership2' ),
			self::COMM_VAR_USER_FIRST_NAME 			=> __( 'Benutzer: Vorname', 'membership2' ),
			self::COMM_VAR_USER_LAST_NAME 			=> __( 'Benutzer: Nachname', 'membership2' ),
			self::COMM_VAR_USERNAME 				=> __( 'Benutzer: Anmeldename', 'membership2' ),
			self::COMM_VAR_PASSWORD 				=> __( 'Benutzer-Passwort', 'membership2' ),
			self::COMM_VAR_RESETURL 				=> __( 'Benutzer: Passwort-URL zurücksetzen', 'membership2' ),
			self::COMM_VAR_MS_ACCOUNT_PAGE_URL 		=> __( 'Webseite: Benutzerkonto-URL', 'membership2' ),
			self::COMM_VAR_BLOG_NAME 				=> __( 'Webseite: Name', 'membership2' ),
			self::COMM_VAR_BLOG_URL 				=> __( 'Webseite: URL', 'membership2' ),
			self::COMM_VAR_VERIFICATION_URL			=> __( 'Benutzer: Kontobestätigungs-URL', 'membership2' ),
		);
		...
		if ( is_multisite() ) {
			$this->comm_vars[ self::COMM_VAR_NET_NAME ] = __( 'Netzwerk: Name', 'membership2' );
			$this->comm_vars[ self::COMM_VAR_NET_URL ] 	= __( 'Netzwerk: URL', 'membership2' );
		}
	}
	...
	public function get_description() {
		$description = __( 'Überschreibe diese Beschreibung in der untergeordneten Klasse', 'membership2' );
	...
	public function set_period_name( $field ) {
		$field['title'] = __( 'Zeitraum vorher/nachher', 'membership2' );
		...
		if ( !$settings->enable_cron_use ) {
			...
			*/
			if ( MS_Plugin::get_modifier( 'MS_STOP_EMAILS' ) ) {
				...
				if ( $subscription ) {
					$msg = sprintf(
						'Die folgende E-Mail wurde nicht gesendet: "%s" an Benutzer "%s".',
						$this->type,
						$subscription->user_id
					);
				} else {
	...
	public function send_message( $reference ) {
		...
		if ( $subscription ) {
			...
			if ( is_numeric( $delay ) && $delay < $ignore_duration ) {
				$msg = sprintf(
					'Überspringe absichtlich doppelte E-Mails: "%s" an Benutzer "%s".',
					$this->type,
					$user_id
				);
				$this->log( $msg );
