...
class MS_Model_Event extends MS_Model_CustomPostType {
	...
	public static function get_register_post_type_args() {
		$args = array(
			'label' 				=> __( 'Mitgliedschaften Ereignisse', 'membership2' ),
	...
	public static function get_event_types() {
		$types = array(
			...
			self::TYPE_MS_REGISTERED => array(
				...
				'desc' 	=> __( 'Hat sich registriert.', 'membership2' ),
			),
			self::TYPE_MS_RESETPASSWORD => array(
				'topic' => self::TOPIC_USER,
				'desc' 	=> __( 'Passwort zurückgesetzt.', 'membership2' ),
			),
			self::TYPE_MS_VERIFYACCOUNT => array(
				'topic' => self::TOPIC_USER,
				'desc' 	=> __( 'Konto verifiziert.', 'membership2' ),
			),
			self::TYPE_UPDATED_INFO => array(
				'topic' => self::TOPIC_USER,
				'desc' 	=> __( 'Hat die Rechnungsinformationen aktualisiert.', 'membership2' ),
			),
			self::TYPE_CREDIT_CARD_EXPIRE => array(
				'topic' => self::TOPIC_USER,
				'desc' 	=> __( "Kreditkarte des Mitglieds läuft ab.", 'membership2' ),
			),
			...
			self::TYPE_MS_SIGNED_UP => array(
				...
				'desc' 	=> __( 'Hat sich für die Mitgliedschaft %s angemeldet.', 'membership2' ),
			),
			self::TYPE_MS_MOVED => array(
				'topic' => self::TOPIC_MEMBERSHIP,
				'desc' 	=> __( 'Ist zur Mitgliedschaft %s gewechselt.', 'membership2' ),
			),
			self::TYPE_MS_EXPIRED => array(
				'topic' => self::TOPIC_MEMBERSHIP,
				'desc' 	=> __( 'Die Mitgliedschaft %s ist abgelaufen.', 'membership2' ),
			),
			self::TYPE_MS_DROPPED => array(
				'topic' => self::TOPIC_MEMBERSHIP,
				'desc' 	=> __( 'Die Mitgliedschaft %s ist gesunken.', 'membership2' ),
			),
			self::TYPE_MS_RENEWED => array(
				'topic' => self::TOPIC_MEMBERSHIP,
				'desc' 	=> __( 'Mitgliedschaft %s erneuert', 'membership2' ),
			),
			self::TYPE_MS_DEACTIVATED => array(
				'topic' => self::TOPIC_MEMBERSHIP,
				'desc' 	=> __( 'Mitgliedschaft %s deaktiviert', 'membership2' ),
			),
			self::TYPE_MS_CANCELED => array(
				'topic' => self::TOPIC_MEMBERSHIP,
				'desc' 	=> __( 'Mitgliedschaft %s gekündigt.', 'membership2' ),
			),
			...
			self::TYPE_MS_BEFORE_FINISHES => array(
				...
				'desc' 	=> __( 'Die Mitgliedschaft %s steht kurz vor dem Ende des Warndatums.', 'membership2' ),
			),

			self::TYPE_MS_AFTER_FINISHES => array(
				'topic' => self::TOPIC_WARNING,
				'desc' 	=> __( 'Mitgliedschaft %s beendet Warnungsdatum.', 'membership2' ),
			),
			self::TYPE_MS_BEFORE_TRIAL_FINISHES => array(
				'topic' => self::TOPIC_WARNING,
				'desc' 	=> __( 'Die Testversion der Mitgliedschaft %s steht kurz vor dem Ablauf.', 'membership2' ),
			),
			...
			self::TYPE_PAID => array(
				...
				'desc' 	=> __( 'Rechnung #%2$s für Mitgliedschaft %1$s - Bezahlt.', 'membership2' ),
			),
			self::TYPE_PAYMENT_FAILED => array(
				'topic' => self::TOPIC_PAYMENT,
				'desc' 	=> __( 'Rechnung #%2$s für Mitgliedschaft %1$s - Zahlung fehlgeschlagen.', 'membership2' ),
			),
			self::TYPE_PAYMENT_PENDING => array(
				'topic' => self::TOPIC_PAYMENT,
				'desc' 	=> __( 'Rechnung #%2$s für Mitgliedschaft %1$s - Zahlung ausstehend.', 'membership2' ),
			),
			self::TYPE_PAYMENT_DENIED => array(
				'topic' => self::TOPIC_PAYMENT,
				'desc' 	=> __( 'Rechnung #%2$s für Mitgliedschaft %1$s - Zahlung verweigert.', 'membership2' ),
			),
			self::TYPE_PAYMENT_BEFORE_DUE => array(
				'topic' => self::TOPIC_PAYMENT,
				'desc' 	=> __( 'Rechnung #%2$s vor Fälligkeit für Mitgliedschaft %1$s Warnung.', 'membership2' ),
			),
			self::TYPE_PAYMENT_AFTER_DUE => array(
				'topic' => self::TOPIC_PAYMENT,
				'desc' 	=> __( 'Rechnung #%2$s nach Fälligkeitsdatum für Mitgliedschaft %1$s Warnung.', 'membership2' ),
			),
		);
			...
			switch ( $event->topic ) {
				...
				case self::TOPIC_USER:
					if ( $data instanceof MS_Model_Member ) {
						...
						$event->ms_relationship_id 	= $subscription->id;
						$event->name 				= sprintf(
														'Benutzer: %s, Mitgliedschaft: %s, Typ: %s',
														$member->name,
														$membership->name,
														$type
													 );
				...
				default:
					MS_Helper_Debug::debug_log( "Ereignistheme nicht implementiert: '$event->topic'" );
					break;
			}
