...
class MS_Model_Invoice extends MS_Model_CustomPostType {
	...
	public static function get_register_post_type_args() {
		$args = array(
			'label' 				=> __( 'Mitgliedschaften Rechnungen', 'membership2' ),
			'description' 			=> __( 'Mitgliedsrechnungen', 'membership2' ),
			'public' 				=> false,
			'show_ui' 				=> false,
			'show_in_menu' 			=> false,
			'has_archive' 			=> false,
	...
	public static function get_status_types( $extended = false ) {
		if ( $extended ) {
			$result = array(
				self::STATUS_NEW 		=> __( 'Entwurf - Rechnung wird vorbereitet, aber der Benutzer kann sie noch nicht sehen', 'membership2' ),
				self::STATUS_BILLED 	=> __( 'In Rechnung gestellt - Der Benutzer kann die Rechnung sehen und muss bezahlen', 'membership2' ),
				self::STATUS_PENDING 	=> __( 'Ausstehend - Warten auf Bestätigung vom Zahlungsgateway', 'membership2' ),
				self::STATUS_PAID 		=> __( 'Bezahlt - Zahlung auf unserem Konto eingegangen!', 'membership2' ),
				self::STATUS_DENIED 	=> __( 'Verweigert - Zahlung abgelehnt', 'membership2' ),
			);
		} else {
			$result = array(
				self::STATUS_NEW 		=> __( 'Entwurf', 'membership2' ),
				self::STATUS_BILLED 	=> __( 'In Rechnung gestellt', 'membership2' ),
				self::STATUS_PENDING 	=> __( 'Ausstehend', 'membership2' ),
				self::STATUS_PAID 		=> __( 'Bezahlt', 'membership2' ),
				self::STATUS_DENIED 	=> __( 'Verweigert', 'membership2' ),
			);
		}
	...
	public function archive() {
		if ( $this->id ) {
			...
			$this->add_notes(
				sprintf(
					__( 'Archiviert am: %s', 'membership2' ),
					MS_Helper_Period::current_date()
				)
			);
			$this->add_notes(
				sprintf(
					__( 'Ehemaliger Status: %s', 'membership2' ),
					$this->status
				)
			);
	...
	public function changed() {
		...
		if ( ! $this->ms_relationship_id ) {
			MS_Helper_Debug::debug_log( 'Transaktion kann nicht verarbeitet werden: Keine Bezug definiert (Inv #' . $this->id  .')' );
		} else {
			$force_admin 	= false;
			$subscription 	= $this->get_subscription();
