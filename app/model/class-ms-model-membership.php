...
class MS_Model_Membership extends MS_Model_CustomPostType {
	...
	public static function get_register_post_type_args() {
		$args = array(
			'label' 				=> __( 'Mitgliedschaften Mitgliedschaften', 'membership2' ),
			'description' 			=> __( 'Mitgliedschaften, denen Benutzer beitreten können.', 'membership2' ),
			'show_ui' 				=> false,
			'show_in_menu' 			=> false,
	...
	public static function get_payment_types( $type = 'paid' ) {
		if ( 'free' == $type ) {
			$payment_types = array(
				self::PAYMENT_TYPE_PERMANENT 	=> __( 'Permanenter Zugang', 'membership2' ),
				self::PAYMENT_TYPE_FINITE	 	=> __( 'Endlicher Zugang', 'membership2' ),
				self::PAYMENT_TYPE_DATE_RANGE 	=> __( 'Zugriff auf den Datumsbereich', 'membership2' ),
			);
		} else {
			$payment_types = array(
				self::PAYMENT_TYPE_PERMANENT 	=> __( 'Eine Zahlung für dauerhaften Zugang', 'membership2' ),
				self::PAYMENT_TYPE_FINITE 		=> __( 'Eine Zahlung für endlichen Zugang', 'membership2' ),
				self::PAYMENT_TYPE_DATE_RANGE 	=> __( 'Eine Zahlung für den Zugriff auf den Datumsbereich', 'membership2' ),
				self::PAYMENT_TYPE_RECURRING 	=> __( 'Wiederkehrende Zahlungen', 'membership2' ),
			);
		}
	...
	public static function get_signup_membership_list(
		...
		if ( $only_names ) {
			...
			if ( ! empty( $private ) ) {
				$priv_key = __( 'Private Mitgliedschaften', 'membership2' );
	...
	public static function _get_system_membership( $type, $create_missing = true ) {
		...
		if ( ! isset( $Special_Membership[ $type ] ) ) {
			...
			if ( ! empty( $base ) ) {
				...
				$description		 		= __( 'Mitgliedschaften Kernmitgliedschaft', 'membership2' );
	...
	public function delete() {
		...
		if ( $this->is_base() ) {
			throw new Exception(
				'Die Systemmitgliedschaft kann nicht gelöscht werden.'
			);
		}
	...
	public function get_payment_type_desc() {
		...
		switch ( $this->payment_type ) {
			case self::PAYMENT_TYPE_FINITE:
				if ( $has_payment ) {
					$desc = sprintf(
						__( 'Zahle für %1$s', 'membership2' ),
						MS_Helper_Period::get_period_desc( $this->period, true )
					);
				} else {
					$desc = sprintf(
						__( 'Frei für %1$s', 'membership2' ),
						MS_Helper_Period::get_period_desc( $this->period, true )
					);
				}
				break;
			...
			case self::PAYMENT_TYPE_DATE_RANGE:
				if ( $has_payment ) {
					$desc = sprintf(
						__( 'Zahle von %1$s bis %2$s', 'membership2' ),
						$this->period_date_start,
						$this->period_date_end
					);
				} else {
					$desc = sprintf(
						__( 'Frei von %1$s bis %2$s', 'membership2' ),
						$this->period_date_start,
						$this->period_date_end
					);
				}
			...
			case self::PAYMENT_TYPE_RECURRING:
				if ( $has_payment ) {
					$desc 		= __( 'Zahle jeweils %1$s', 'membership2' );
					if ( 1 == $this->pay_cycle_repetitions ) {
						$desc 	= __( 'Einmalzahlung', 'membership2' );
					} elseif ( $this->pay_cycle_repetitions > 1 ) {
						$desc 	.= ', ' . __( '%2$s Zahlungen', 'membership2' );
					}
				} else {
					$desc 		= __( 'Freier Zugang', 'membership2' );
				}
	...
	public function get_name_tag( $with_tooltip = false ) {
		if ( $with_tooltip ) {
			$tooltip = sprintf(
				__( 'Revision: %s | Typ: %s', 'membership2' ),
				$this->revision,
				$this->type
			);
			$tag = sprintf(
	...
	public function get_after_ms_ends_options() {
		$options = array(
			0 => __( 'Beschränke den Zugriff auf Besucherebene', 'membership2' ),
		);
		...
		$label = __( 'Ändern zu: %s', 'membership2' );
			...
			foreach ( $rules as $rule ) {
				...
				if ( null === $rule_access ) {
					$this->_access_reason[] = sprintf(
						__( 'Ignoriert: Regel "%s"', 'membership2' ),
						$rule->rule_type
					);
					continue;
				}
				...
				$this->_access_reason[] = sprintf(
					__( '%s: Regel "%s"', 'membership2' ),
					$rule_access ? __( 'Erlauben', 'membership2' ) : __( 'Verweigern', 'membership2' ),
					$rule->rule_type
				);
			...
			switch ( $property ) {
				...
				case 'payment_type':
					...
					if ( array_key_exists( $value, $types ) ) {
						...
						throw new Exception( 'Ungültiger Mitgliedschaftstyp.' );
					}
					break;
