...
class MS_Model_Import_Membership extends MS_Model_Import {
	...
	public function prepare() {
		...
		if ( empty( $this->data ) ) {
			self::_message( 'error', __( 'Hmmm, wir konnten die Mitgliedschaftsdaten nicht importieren...', 'membership2' ) );
			return false;
		}
	...
	protected function prepare_import_struct() {
		...
		$this->data->notes 			= array(
			__( 'Exportierte Daten:', 'membership2' ),
			__( '- Abonnementpläne (ohne Levelregeln)', 'membership2' ),
			__( '- Mitglieder', 'membership2' ),
			__( '- Registrierungen (Link zwischen Mitgliedern und Abonnementplänen)', 'membership2' ),
			__( '- Transaktionen', 'membership2' ),
			__( 'Jede Abonnementstufe wird als einzelne Mitgliedschaft importiert.', 'membership2' ),
			__( 'Transaktionen werden in Rechnungen umgewandelt. Daten wie Steuersatz oder angewandte Gutscheine sind nicht verfügbar.', 'membership2' ),
			__( 'Bitte beachte, dass wir keine wiederkehrenden 2Checkout-Abonnements in Mitgliedschaften importieren können!', 'membership2' ),
		);
