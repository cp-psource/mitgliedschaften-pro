...
class MS_Model_Member extends MS_Model {
	...
	public static function get_usernames( $args = null, $search_option = self::SEARCH_ONLY_MEMBERS, $return_array = true ) {
		...
		if ( $return_array ) {
			$members[0] = __( 'Wähle einen Benutzer aus', 'membership2' );
		}
	...
	private function create_new_user() {
		...
		if ( is_user_logged_in() ) {
			throw new Exception( __( 'Du kannst kein neues Konto registrieren, da Du bereits angemeldet bist.', 'membership2' ), 1 );
			return;
		}
		...
		$required = array(
			...
			'email' 		=> __( 'E-Mail-Addresse', 'membership2' ),
			'password'   	=> __( 'Passwort', 'membership2' ),
			'password2'  	=> __( 'Passwort Bestätigung', 'membership2' ),
		);
		...
		foreach ( $required as $field => $message ) {
			if ( empty( $this->$field ) && empty( $_POST[$field] ) ) {
				$validation_errors->add(
					$field,
					sprintf(
						__( 'Stelle sicher, dass die Informationen <span class="ms-bold">%s</span> vollständig sind.', 'membership2' ),
						$message
					)
				);
			}
		...
		if ( $this->password != $this->password2 ) {
			$validation_errors->add(
				'passmatch',
				__( 'Bitte stelle sicher, dass die Passwörter übereinstimmen.', 'membership2' )
			);
		}
		...
		if ( ! validate_username( $this->username ) ) {
			$validation_errors->add(
				'usernamenotvalid',
				__( 'Der Benutzername ist leider nicht gültig.', 'membership2' )
			);
		}
		...
		if ( username_exists( $this->username ) ) {
			$validation_errors->add(
				'usernameexists',
				__( 'Dieser Benutzername ist leider bereits vergeben.', 'membership2' )
			);
		}
		...
		if ( ! is_email( $this->email ) ) {
			$validation_errors->add(
				'emailnotvalid',
				__( 'Die E-Mail-Adresse ist leider nicht gültig.', 'membership2' )
			);
		}
		...
		if ( email_exists( $this->email ) ) {
			$validation_errors->add(
				'emailexists',
				__( 'Diese E-Mail-Adresse ist leider bereits vergeben.', 'membership2' )
			);
		}
		...
		if ( is_multisite() ) {
			...
			if ( $limited_domains && is_array( $limited_domains ) ) {
				if ( ! in_array( $email_domain, $limited_domains ) ) {
					$validation_errors->add(
						'emaildomain',
						__( 'Diese E-Mail-Domain darf leider nicht registriert werden.', 'membership2' )
					);
				}
			}
		...
		if ( MS_Helper_Utility::get_privacy_page() && empty( $_POST['privacy_check'] ) ) {
			$validation_errors->add(
				'privacy_check',
				__( 'Du musst die Datenschutzbestimmungen lesen und akzeptieren.', 'membership2' )
			);
		}
	...
	public function signon_user() {
		...
		if ( ! headers_sent() ) {
			...
			if ( is_wp_error( $user ) ) {
				$this->log( 'Bei der Anmeldung ist ein Fehler aufgetreten '.$user->get_error_message() );
				return $user;
			}
		}
	...
	public static function verify_activation_code( $user_id ) {
		if ( $user_id ) {
			...
			if ( $user_activation_status != 1 ) {
				...
				return __( 'Konto verifiziert. Fahre mit der Anmeldung fort', 'membership2' );
			} else {
				return __( 'Konto bereits verifiziert', 'membership2' );
			}
			
		} else {
			return __( 'Ungültiger Code. Bitte überprüfe Deine E-Mails oder versuche es erneut', 'membership2' );
		}
	}
