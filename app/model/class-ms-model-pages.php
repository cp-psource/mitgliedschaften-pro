...
class MS_Model_Pages extends MS_Model_Option {
	...
	static public function get_page_types() {
		...
		if ( empty( $Page_types ) ) {
			$Page_types = array(
				...
				self::MS_PAGE_ACCOUNT 			=> __( 'Konto', 'membership2' ),
			);
	...
	static public function get_description( $type ) {
		...
		if ( null === $Description ) {
			$Description = array(
				self::MS_PAGE_MEMBERSHIPS		=> __( '(Nur für angemeldete Benutzer) Liste die öffentlichen Mitgliedschaften und die Anmeldung auf.', 'membership2' ),
				self::MS_PAGE_PROTECTED_CONTENT => __( 'Wird angezeigt, wenn ein Benutzer nicht auf die angeforderte Seite zugreifen kann.', 'membership2' ),
				self::MS_PAGE_REGISTER 			=> __( '(Nur für Gäste) Liste die öffentlichen Mitgliedschaften und das Anmeldeformular auf.', 'membership2' ),
				self::MS_PAGE_REG_COMPLETE 		=> __( 'Wird angezeigt, nachdem sich der Benutzer für eine Mitgliedschaft angemeldet hat.', 'membership2' ),
				self::MS_PAGE_ACCOUNT 			=> __( 'Zeigt Details zum aktuellen Benutzer an.', 'membership2' ),
			);
		...
		switch ( $type ) {
			case self::MS_PAGE_MEMBERSHIPS:
				$lines[] = sprintf(
					...
					__( 'Wir haben die folgenden Abonnements für unsere Webseite verfügbar. Du kannst Deine Abonnements mithilfe der folgenden Formulare verlängern, kündigen oder aktualisieren.', 'membership2' )
				);
			...
			case self::MS_PAGE_REGISTER:
				$lines[] = sprintf(
					'['. MS_Helper_Shortcode::SCODE_NOTE .' type="info"]%1$s[/'. MS_Helper_Shortcode::SCODE_NOTE .']',
					__( 'Wir haben die folgenden Abonnements für unsere Webseite verfügbar. Um teilzunehmen, klicke einfach auf die Schaltfläche Anmelden und vervollständige die Registrierungsdetails.', 'membership2' )
				);
				$lines[] = '['. MS_Helper_Shortcode::SCODE_SIGNUP .']';
				break;
			...
			case self::MS_PAGE_REG_COMPLETE:
				$lines[] = sprintf(
					...
					__( 'Deine Anfrage zur Mitgliedschaft wurde erfolgreich entgegengenommen!', 'membership2' ),
					__( 'Das Payment Gateway kann einige Minuten benötigen, um den Zahlungsstatus zu verarbeiten und zurückzugeben.', 'membership2' )
				);
	...
	static public function create_default_menu() {
		...
		$menu_id = wp_create_nav_menu( __( 'Standardmenü', 'membership2' ) );
