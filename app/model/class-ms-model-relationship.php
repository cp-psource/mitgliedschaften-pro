...
class MS_Model_Relationship extends MS_Model_CustomPostType {
	...
	public static function get_register_post_type_args() {
		$args = array(
			'label' 				=> __( 'Mitgliedschaften-Abonnements', 'membership2' ),
	...
	public static function get_status_types() {
		$status_types = array(
			self::STATUS_PENDING 		=> __( 'Ausstehend', 'membership2' ),
			self::STATUS_ACTIVE 		=> __( 'Aktiv', 'membership2' ),
			self::STATUS_TRIAL 			=> __( 'Test', 'membership2' ),
			self::STATUS_TRIAL_EXPIRED 	=> __( 'Test abgelaufen', 'membership2' ),
			self::STATUS_EXPIRED 		=> __( 'Abgelaufen', 'membership2' ),
			self::STATUS_DEACTIVATED 	=> __( 'Deaktiviert', 'membership2' ),
			self::STATUS_CANCELED 		=> __( 'Abgebrochen', 'membership2' ),
			self::STATUS_WAITING 		=> __( 'Noch nicht aktiv', 'membership2' ),
		);
	...
	public function get_payment_description( $invoice = null, $short = false ) {
		...
		switch ( $payment_type ) {
			case MS_Model_Membership::PAYMENT_TYPE_PERMANENT:
				if ( 0 == $total_price ) {
					if ( $short ) {
						$lbl = __( 'Nichts (für immer)', 'membership2' );
					} else {
						$lbl = __( 'Für den dauerhaften Zugang zahlst Du nichts.', 'membership2' );
					}
				} else {
					if ( $short ) {
						$lbl  = __( '<span class="price">%1$s %2$s</span> (für immer)', 'membership2' );
					} else {
						$lbl = __( 'Du bezahlst <span class="price">%1$s %2$s</span> für den permanenten Zugriff.', 'membership2' );
					}
				}

				if ( MS_Model_Member::is_admin_user() ) {
					$desc = __( 'Admin hat keine Gebühren!', 'membership2' );
				} else {
					$desc = sprintf(
						$lbl,
						$currency,
			...
			case MS_Model_Membership::PAYMENT_TYPE_FINITE:
				if ( 0 == $total_price ) {
					if ( $short ) {
						$lbl = __( 'Nichts (bis %4$s)', 'membership2' );
					} else {
						$lbl = __( 'Du bezahlst nichts für den Zugang bis %3$s.', 'membership2' );
					}
				} else {
					if ( $short ) {
						$lbl = __( '<span class="price">%1$s %2$s</span> (bis %4$s)', 'membership2' );
					} else {
						$lbl = __( 'Du bezahlst <span class="price">%1$s %2$s</span> für den Zugriff bis %3$s.', 'membership2' );
					}
				}
			...
			case MS_Model_Membership::PAYMENT_TYPE_DATE_RANGE:
				if ( 0 == $total_price ) {
					if ( $short ) {
						$lbl = __( 'Nichts (%5$s - %6$s)', 'membership2' );
					} else {
						$lbl = __( 'Du bezahlst nichts für den Zugriff von %3$s bis %4$s.', 'membership2' );
					}
				} else {
					if ( $short ) {
						$lbl = __( '<span class="price">%1$s %2$s</span> (%5$s - %6$s)', 'membership2' );
					} else {
						$lbl = __( 'Du bezahlst <span class="price">%1$s %2$s</span> für den Zugriff von %3$s bis %4$s.', 'membership2' );
					}
				}
			...
			case MS_Model_Membership::PAYMENT_TYPE_RECURRING:
				...
				if ( 1 == $membership->pay_cycle_repetitions ) {
					...
					if ( $short ) {
						$lbl = __( '<span class="price">%1$s %2$s</span> (einmalig)', 'membership2' );
					} else {
						$lbl = __( 'Du bezahlst <span class="price">%1$s %2$s</span> einmalig.', 'membership2' );
					}
				} else {
					if ( $membership->pay_cycle_repetitions > 1 ) {
						// Fixed number of payments (more than 1)
						if ( $short ) {
							$lbl = _n( 'Einmal <span class="price">%1$s %2$s</span>.', '%4$s mal <span class="price">%1$s %2$s</span> (alle %3$s)', $no_payment_rest, 'membership2' );
						} else {
							$lbl = _n( 'Du leistest eine Zahlung von <span class="price">%1$s %2$s</span>.', 'Du leistest %4$s Zahlungen von <span class="price">%1$s %2$s</span>, einmal alle %3$s.', $no_payment_rest, 'membership2' );
						}
					} else {
						// Indefinite number of payments
						if ( $short ) {
							$lbl = __( '<span class="price">%1$s %2$s</span> (alle %3$s)', 'membership2' );
						} else {
							$lbl = __( 'Du bezahlst <span class="price">%1$s %2$s</span> alle %3$s.', 'membership2' );
						}
					}
				}
			...
			if ( 0 == absint( $trial_price ) ) {
				if ( $short ) {
					if ( MS_Model_Membership::PAYMENT_TYPE_RECURRING == $payment_type ) {
						$lbl = __( 'nach %4$s', 'membership2' );
					} else {
						$lbl = __( 'am %4$s', 'membership2' );
					}
				} else {
					$trial_price 	= __( 'nichts', 'membership2' );
					$lbl 			= __( 'Deine kostenlose Testversion von %1$ endet mit %5$ und dann wird Dir eine Rechnung ausgestellt.', 'membership2' );
				}
			} else {
				$trial_price 	= MS_Helper_Billing::format_price( $trial_price );
				$lbl 			= __( 'Für den Testzeitraum von %1$s bezahlst Du nur <span class="price">%2$s %3$s</span>.', 'membership2' );
			}
	...
	protected function calculate_status( $set_status = null, $debug = false ) {
		...
		if ( $can_activate ) {
			...
			$debug_msg[] = '[AUSSTEHEND: Ausstehendes Abonnement kann nicht aktiviert werden]';
		} elseif ( ! $calc_status && $debug ) {
			$debug_msg[] = '[Nicht AKTIV/ANHÄNGIG: Abonnement kann nicht aktiviert werden]';
		}

		// If no other condition was true then the expire date was reached.
		if ( ! $calc_status ) {
			$calc_status = self::STATUS_EXPIRED;
			$debug_msg[] = '[ABGELAUFEN: Standardstatus]';
		}
	...
	public function get_status_description() {
		...
		switch ( $this->status ) {
			case self::STATUS_PENDING:
				$desc = __( 'Ausstehende Zahlung.', 'membership2' );
				break;

			case self::STATUS_TRIAL:
				$desc = sprintf(
					'%s <span class="ms-date">%s</span>',
					__( 'Die Mitgliedschaftsprüfung läuft am ab ', 'membership2' ),
			...
			case self::STATUS_ACTIVE:
				if ( ! empty( $this->expire_date ) ) {
					$desc = sprintf(
						'%s <span class="ms-date">%s</span>',
						__( 'Die Mitgliedschaft läuft am ab ', 'membership2' ),
						MS_Helper_Period::format_date( $this->expire_date )
					);
				} else {
					$desc = __( 'Permanenter Zugang.', 'membership2' );
				}
				break;
			...
			case self::STATUS_EXPIRED:
				$desc = sprintf(
					'%s <span class="ms-date">%s</span>',
					__( 'Die Mitgliedschaft ist seitdem abgelaufen ', 'membership2' ),
					MS_Helper_Period::format_date( $this->expire_date )
				);
				break;
			...
			case self::STATUS_CANCELED:
				$desc = sprintf(
					'%s <span class="ms-date">%s</span>',
					__( 'Die Mitgliedschaft wurde gekündigt und ist bis zum Ablauf gültig ', 'membership2' ),
					MS_Helper_Period::format_date( $this->expire_date )
				);
				break;

			case self::STATUS_DEACTIVATED:
				$desc = __( 'Mitgliedschaft deaktiviert.', 'membership2' );
				break;
		}
		...
		switch ( $property ) {
			...
			default:
				if ( ! property_exists( $this, $property ) ) {
					MS_Helper_Debug::debug_log( 'Eigentum existiert nicht: ' . $property );
				} else {
					$value = $this->$property;
				}
				break;
