...
class MS_Helper_Media extends MS_Helper {
	...
	public static function write_htaccess_rule( $files = array() ) {
		if ( !empty( $files ) && is_array( $files ) ) {
			$new_rule = array(
				PHP_EOL. "## Mitgliedschaften - Medienschutz ##" . PHP_EOL,
    ...
    public static function clear_htaccess() {
        ...
        if ( file_exists( $ht_file ) ) {
            if ( ! is_writeable( $ht_file ) ) {
                return new WP_Error( 'not_writeable', __( 'Die .htaccess-Datei ist nicht beschreibbar', 'membership2' ) );
            } else {
                $ht_content = file_get_contents( $ht_file );
                if ( !empty( $ht_content) ) {
                    preg_match_all('/## Membership 2(.*?)## Mitgliedschaften - Ende ##/s', $ht_content, $matches);
