...
class MS_Helper_ListTable {
	...
	public function no_items() {
		if ( $this->is_search() ) {
			_e( 'Keine Elemente gefunden.', 'membership2' );
		} else {
			_e( 'Keine Artikel verfügbar.', 'membership2' );
		}
	}
	...
	public function search_box( $text = null, $input_id = 'search' ) {
		...
		if ( $this->need_pagination() || $this->is_search() ) {
			if ( empty( $text ) ) {
				$text = __( 'Suche', 'membership2' );
			} else {
				$text = sprintf(
					__( 'Suche %1$s', 'membership2' ),
					$text
				);
			}
			...
			if ( $this->is_search() ) {
				...
				<span class="ms-search-info">
					<?php
					printf(
						__( 'Suchergebnisse für &#8220;%s&#8221;', 'membership2' ),
						sprintf(
	...
	public function bulk_actions( $echo = true ) {
		...
		printf( '<option value="-1" selected="selected">%s</option>', __( 'Massenaktionen', 'membership2' ) );
	...
	protected function months_dropdown( $post_type ) {
		...
		?>
		<select name='m'>
			<option<?php selected( $m, 0 ); ?> value='0'><?php _e( 'Alle Daten anzeigen', 'membership2' ); ?></option>
		<?php
	...
	protected function view_switcher( $current_mode ) {
		$modes = array(
			'list'    => __( 'Listenansicht', 'membership2' ),
			'excerpt' => __( 'Auszugsansicht', 'membership2' ),
		);
	...
	protected function comments_bubble( $post_id, $pending_comments ) {
		$pending_phrase = sprintf( __( '%s ausstehend', 'membership2' ), number_format( $pending_comments ) );
	...
	protected function pagination( $which, $echo = true ) {
		...
		$output = '<span class="displaying-num">' .
			sprintf(
				_n( '1 Element', '%s Elemente', $total_items, 'membership2' ),
		...
		if ( $this->need_pagination() && ! $this->is_search() ) {
			...
			$page_links[] 	= sprintf(
				...
				esc_attr__( 'Gehe zur ersten Seite', 'membership2' ),
			...
			$page_links[] 	= sprintf(
				...
				esc_attr__( 'Gehe zur vorherigen Seite', 'membership2' ),
			...
			if ( 'bottom' == $which ) {
				...
				$html_current_page = sprintf(
					...
					esc_attr__( 'Aktuelle Seite', 'membership2' ),
					$current,
					strlen( $total_pages )
				);
			}
			...
			$page_links[] 		= '<span class="paging-input">' . sprintf( _x( '%1$s von %2$s', 'paging', 'membership2' ), $html_current_page, $html_total_pages ) . '</span>';

			$page_links[] 		= sprintf(
				'<a class="%s" title="%s" href="%s">%s</a>',
				'next-page' . $disable_last,
				esc_attr__( 'Gehe zur nächsten Seite', 'membership2' ),
			...
			$page_links[] 		= sprintf(
				...
				esc_attr__( 'Gehe zur letzten Seite', 'membership2' ),
	...
	protected function get_columns() {
		die( 'Die Funktion WP_ListTable::get_columns() muss in einer Unterklasse überschrieben werden.' );
	}
	...
	protected function display_rows_or_placeholder() {
		if ( $this->has_items() ) {
			...
			<p class="submit inline-edit-save">
				<a accesskey="c" href="#inline-edit" class="button-secondary cancel alignleft"><?php _e( 'Abbrechen', 'membership2' ); ?></a>
				<input type="hidden" id="_wpnonce" name="_wpnonce" value="<?php echo esc_attr( $inline_nonce ); ?>">
				<a accesskey="s" href="#inline-edit" class="button-primary save alignright"><?php _e( 'Aktualisieren', 'membership2' ); ?></a>
	...
	public function ajax_response() {
		...
		if ( isset( $total_items ) ) {
			$response['total_items_i18n'] 	= sprintf( _n( '1 Element', '%s Elemente', $total_items, 'membership2' ), number_format_i18n( $total_items ) );
		}
