...
class MS_Controller_Gateway extends MS_Controller {
	...
	public function process_purchase() {
		...
		if ( $valid ) {
			...
			MS_Helper_Debug::debug_log( 'Fehlercode ' . $err );
	...
	public function purchase_error_content( $content ) {
		return apply_filters(
			...
			__( 'Entschuldigung, Deine Anmeldeanforderung ist fehlgeschlagen. Versuch es noch einmal.', 'membership2' ),
			$content,
			$this
		);
	}
		...
		if ( ! empty( $wp_query->query_vars['paymentgateway'] ) ) {
			...
			if ( MS_Model_Gateway::is_active( $gateway ) ) {
				...
				if ( MS_Model_Gateway::is_valid_gateway( $gateway ) ) {
					$note = __( 'Gateway ist inaktiv', 'membership2' );
				} else {
					$note = sprintf(
						__( 'Unbekanntes Gateway: %s', 'membership2' ),
						$gateway
					);
				}
	...
	public function handle_webhook( $wp_query ) {
		...
		if ( ! empty( $wp_query->query_vars['mswebhook'] ) ) {
			...
			if ( MS_Model_Gateway::is_active( $gateway ) ) {
				...
				// Log the payment attempt when the gateway is not active.
				if ( MS_Model_Gateway::is_valid_gateway( $gateway ) ) {
					$note = __( 'WebHook: Gateway ist inaktiv', 'membership2' );
				} else {
					$note = sprintf(
						__( 'WebHook: Unbekanntes Gateway: %s', 'membership2' ),
						$gateway
					);
				}
