...
class MS_Controller_Shortcode extends MS_Controller {
		...
		$data = apply_filters(
			...
			shortcode_atts(
				array(
					...
					'label_first_name' 	=> __( 'Vorname', 'membership2' ),
					'label_last_name' 	=> __( 'Nachname', 'membership2' ),
					'label_username' 	=> __( 'Wähle einen Benutzernamen', 'membership2' ),
					'label_email' 		=> __( 'E-Mail-Addresse', 'membership2' ),
					'label_password' 	=> __( 'Passwort', 'membership2' ),
					'label_password2' 	=> __( 'Passwort bestätigen', 'membership2' ),
					'label_register' 	=> __( 'Registriere mein Konto', 'membership2' ),
					'hint_first_name' 	=> '',
					'hint_last_name' 	=> '',
					'hint_username' 	=> '',
					'hint_email' 		=> '',
					'hint_password' 	=> '',
					'hint_password2' 	=> '',
					'title' 			=> __( 'Ein Konto erstellen', 'membership2' ),
					'loginlink' 		=> true,
				),
				$atts
			)
	...
	public function membership_signup( $atts ) {
		...
		$data = apply_filters(
			...
			shortcode_atts(
				array(
					MS_Helper_Membership::MEMBERSHIP_ACTION_SIGNUP . '_text' 	=> __( 'Anmelden', 'membership2' ),
					MS_Helper_Membership::MEMBERSHIP_ACTION_MOVE . '_text' 		=> __( 'Ändern', 'membership2' ),
					MS_Helper_Membership::MEMBERSHIP_ACTION_CANCEL . '_text' 	=> __( 'Abbrechen', 'membership2' ),
					MS_Helper_Membership::MEMBERSHIP_ACTION_RENEW . '_text' 	=> __( 'Erneuern', 'membership2' ),
					MS_Helper_Membership::MEMBERSHIP_ACTION_PAY . '_text' 		=> __( 'Vollständige Bezahlung', 'membership2' ),
				),
				$atts
			)
		);
		...
		if ( empty( $memberships ) ) {
			...
			return $this->ms_note(
				array( 'type' => 'error' ),
				__( 'Leider sind keine Mitgliedschaften für die Registrierung verfügbar.', 'membership2' )
			);
		}
	...
	public function membership_title( $atts ) {
		...
		$data = apply_filters(
			...
			shortcode_atts(
				array(
					'id' 	=> 0,
					'label' => __( 'Titel der Mitgliedschaft:', 'membership2' ),
	...
	public function membership_buy( $atts ) {
		...
		if ( ! empty( $id ) ) {
			...
			if ( ! $membership->active ){
				return __( 'Es tut uns leid! Die Mitgliedschaft, die Du registrieren möchtest, ist nicht aktiv.', 'membership2' );
			}
	...
	public function membership_details( $atts ) {
		...
		$data = apply_filters(
			...
			shortcode_atts(
				array(
					'id' 	=> 0,
					'label' => __( 'Details zur Mitgliedschaft:', 'membership2' ),
				),
				$atts
			)
		);
		...
		$data = apply_filters(
			...
			shortcode_atts(
				array(
					...
					'label_log_in'    => __( 'Einloggen', 'membership2' ),
		...
		$data = apply_filters(
			...
			shortcode_atts(
				array(
					...
					'membership_title' 			=> __( 'Deine Mitgliedschaft', 'membership2' ),
					'membership_change_label' 	=> __( 'Ändern', 'membership2' ),
					'show_profile' 				=> true,
					'show_profile_change' 		=> true,
					'profile_title' 			=> __( 'Persönliche Daten', 'membership2' ),
					'profile_change_label' 		=> __( 'Bearbeiten', 'membership2' ),
					'show_invoices' 			=> true,
					'limit_invoices' 			=> 10,
					'show_all_invoices' 		=> true,
					'invoices_title' 			=> __( 'Rechnungen', 'membership2' ),
					'invoices_details_label' 	=> __( 'Alle ansehen', 'membership2' ),
					'show_activity' 			=> true,
					'limit_activities' 			=> 10,
					'show_all_activities' 		=> true,
					'activity_title'			=> __( 'Aktivitäten', 'membership2' ),
					'activity_details_label' 	=> __( 'Alle ansehen', 'membership2' ),
				),
				$atts
			)
		);
	...
	public function membership_account_link( $atts ) {
		...
		$data = apply_filters(
			...
			shortcode_atts(
				array(
					'label' => __( 'Besuche Deine Kontoseite für weitere Informationen.', 'membership2' ),
				),
				$atts
			)
		);
	...
	public function ms_no_value( $atts, $content = '' ) {
		...
		if ( MS_Model_Member::is_admin_user() ) {
			$content = sprintf(
				...
				__(
					'Der Inhaltsschutz ist deaktiviert. Bitte aktiviere den Schutz, um diesen Shortcode anzuzeigen.',
					'membership2'
				),
				__(
					'Diese Nachricht wird nur Webseiten-Administratoren angezeigt',
					'membership2'
				)
			);
		} else {
