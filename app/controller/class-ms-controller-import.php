...
class MS_Controller_Import extends MS_Controller {
		...
		if ( $this->verify_nonce()
			...
			if ( MS_Model_Import::match_with_source( $match_id, $source_id, $source ) ) {
				wp_send_json_success(
					array(
						'message' => __( 'Übereinstimmende Details gespeichert. Zukünftige Transaktionen werden ab sofort automatisch bearbeitet!', 'membership2' ),
					)
				);
			}
		}
		...
		switch ( $action ) {
			...
			case self::ACTION_PREVIEW:
				...
				try {
					...
					self::_message(
						'error',
						__( 'In Kürze verfügbar: Diese Importquelle wird noch nicht unterstützt...', 'membership2' )
					);
				}
	...
	public function enqueue_scripts() {
		$data = array(
			...
			'lang' 		=> array(
				'progress_title' 			=> __( 'Daten importieren...', 'membership2' ),
				...
				'task_import_membership' 	=> __( 'Mitgliedschaft importieren', 'membership2' ),
				'task_import_settings' 		=> __( 'Einstellungen importieren', 'membership2' ),
			),
		);
