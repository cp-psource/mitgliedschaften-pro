...
class MS_Rule_Shortcode_Model extends MS_Rule {
	...
	public function do_protected_shortcode() {
		...
		if ( $msg ) {
			...
			$content = __( 'Shortcode-Inhalt geschützt.', 'membership2' );
		}
			...
			if ( self::is_member_of( $membership_ids ) ) {
				...
				if ( $silent ) {
					...
					if ( ! empty( $msg ) ) {
						...
						$content .= __( 'Kein Zugriff für Mitgliedern von: ', 'membership2' );
			...
			if ( ! self::is_member_of( $membership_ids ) ) {
				...
				if ( $silent ) {
					...
					if ( ! empty( $msg ) ) {
						...
						$content .= __( 'Inhalt geschützt für Mitglieder von: ', 'membership2' );
						$content .= implode( ', ', $membership_names );
					}
					$content .= '</div>';
				}
	...
	public static function debug_protect_content_shortcode( $atts, $content = '' ) {
		...
		if ( mslib3()->is_true( $access ) ) {
			$msg_access = __( 'Sichtbar für Mitglieder von', 'membership2' );
			$alt_msg1 	= __( 'Andere Benutzer werden sehen', 'membership2' );
			$alt_msg2 	= __( 'Andere Benutzer sehen nichts', 'membership2' );
		} else {
			$msg_access = __( 'Versteckt vor Mitgliedern von', 'membership2' );
			$alt_msg1 	= __( 'Diese Benutzer werden sehen', 'membership2' );
			$alt_msg2 	= __( 'Diese Verwendungen werden nichts sehen', 'membership2' );
		}
		...
		$code = sprintf(
			...
			$msg_access,
			implode( ' ', $memberships ),
			__( 'Diese Informationen werden nur für Administratorbenutzer angezeigt', 'membership2' )
		);
