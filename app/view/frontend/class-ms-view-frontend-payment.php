...
class MS_View_Frontend_Payment extends MS_View {
	...
	public function to_html() {
		...
		$msg = __(
			'Bitte überprüfe die Details der Mitgliedschaft unten und klicke auf die entsprechende Schaltfläche, um die Anmeldung abzuschließen.', 'membership2'
		);
			...
			if ( ! MS_Model_Addon::is_enabled( MS_Model_Addon::ADDON_MULTI_MEMBERSHIPS ) ) {
				...
				foreach ( $this->data['member']->subscriptions as $tmp_subscription ) {
					...
					if ( in_array( $tmp_subscription->status, $valid_status ) ) {
						$cancel_warning = __(
							'Deine anderen Mitgliedschaften werden gekündigt, wenn Du diese Zahlung abschließt.',
							'membership2'
						);
						break;
					}
				...
				if ( 1 == count( $names ) ) {
					$cancel_warning = sprintf(
						__( 'Wenn Du diese Zahlung abschließt, wird Deine Mitgliedschaft "%s" gekündigt.', 'membership2' ),
						$names[0]
					);
				} elseif ( 1 < count( $names ) ) {
					$cancel_warning = sprintf(
						__( 'Wenn Du diese Zahlung abschließt, werden die folgenden Mitgliedschaften gekündigt: %s.', 'membership2' ),
						implode( ', ', $names )
					);
				}
			}
