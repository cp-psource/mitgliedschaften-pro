...
class MS_View_Protection extends MS_View {
	...
	public function to_html() {
		...
		$desc = array(
			__( 'Wähle Seiten, Kategorien usw. aus, die für Besucher und Nichtmitglieder <strong>nicht verfügbar</strong> sein sollen.', 'membership2' ),
		);
		...
		<div class="ms-wrap wrap ms-wrap-membership2">
			...
			MS_Helper_Html::settings_header(
				array(
					'title' => __( 'Einrichtung der Schutzregeln', 'membership2' ),
					'desc' => $desc,
				)
			);
			...
			if ( is_callable( $render_callback ) ) {
				...
				$html = '<div class="ms-settings">' .
					...
					'<em>Keine Ansicht definiert durch Hook "ms_view_protectedcontent_define-' . $active_tab . '"</em>' .
					'</p></div>' .
					'</div>';
			}
	...
	protected function site_filter() {
		...
		<div class="ms-tab-container">
			...
			<?php _e( 'Seite wählen', 'membership2' ); ?>
			</label>
		</div>
		<div>
