...
class MS_View_Membership_Overview_Simple extends MS_View {
	...
	public function to_html() {
		...
		$edit_button = sprintf(
			...
			'<i class="wpmui-fa wpmui-fa-pencil"></i> ' . __( 'Bearbeiten', 'membership2' )
		);
		...
		<div class="wrap ms-wrap ms-membership-overview">
			<div class="ms-wrap-top ms-group">
				<div class="ms-membership-status-wrapper">
					...
					<div id="ms-membership-status" class="ms-membership-status <?php echo esc_attr( $status_class ); ?>">
						...
						printf(
							...
							sprintf(
								__( 'Mitgliedschaft ist %s', 'membership2' ),
								'<span id="ms-membership-status-text" class="ms-ok">' .
								__( 'Aktiv', 'membership2' ) .
								'</span>'
							)
						);
						printf(
				...
				$title = sprintf(
					__( '%s Überblick', 'membership2' ),
					$membership->get_name_tag( true )
				);
				$desc = array(
					__( 'Hier findest Du eine Zusammenfassung dieser Mitgliedschaft und kannst deren Details ändern.', 'membership2' ),
					sprintf(
						__( 'Dies ist ein %s', 'membership2' ),
	...
	public function news_panel() {
		...
		<div class="ms-half ms-settings-box">
			<h3>
				...
				<?php _e( 'Neueste Nachrichten', 'membership2' ); ?>
			</h3>
				...
				<div class="ms-news-view-wrapper">
					...
					MS_Helper_Html::html_element(
						array(
							...
							'value' => __( 'Weitere Nachrichten anzeigen', 'membership2' ),
							'url' => $url,
			...
			<?php else : ?>
				<div class="inside group">
					<p class="ms-italic">
					<?php _e( 'Es wird hier einige interessante Neuigkeiten geben, wenn Deine Webseite in Betrieb genommen wird.', 'membership2' ); ?>
					</p>
				</div>
			<?php endif; ?>
		</div>
	...
	public function members_panel() {
		...
		<div class="ms-half ms-settings-box">
			<h3>
				...
				<?php printf( __( 'Neue Mitglieder (%s)', 'membership2' ), $count ); ?>
			</h3>
				...
				<div class="ms-member-edit-wrapper">
					...
					MS_Helper_Html::html_element(
						array(
							'id' => 'edit_members',
							'type' => MS_Helper_Html::TYPE_HTML_LINK,
							'value' => __( 'Alle Mitglieder', 'membership2' ),
							'url' => $url,
							'class' => 'wpmui-field-button button',
						)
					);
			...
			<?php else : ?>
				<div class="inside group">
					<p class="ms-italic">
					<?php _e( 'Noch keine Mitglieder.', 'membership2' ); ?>
					</p>
				</div>
			<?php endif; ?>
		</div>
	...
	protected function news_panel_data( $items ) {
		...
		<table class="ms-list-table widefat">
			<thead>
				<tr>
					<th><?php _e( 'Datum', 'membership2' ); ?></th>
					<th><?php _e( 'Mitglied', 'membership2' ); ?></th>
					<th><?php _e( 'Ereignis', 'membership2' ); ?></th>
				</tr>
			</thead>
			<tbody>
		...
		<div class="ms-overview-container">
			<div class="ms-settings">
				...
				<div class="ms-overview-available-content-wrapper ms-overview-bottom">
					<h3><i class="ms-img-unlock"></i> <?php _e( 'Verfügbarer Inhalt', 'membership2' ); ?></h3>
					<div class="ms-description ms-indented-description">
					<?php
					printf(
						__( 'Dies ist Mitgliedschaften, auf die <span class="ms-bold">%s</span>-Mitglieder Zugriff haben.', 'membership2' ),
		...
		if ( ! $membership->is_free ) {
			...
			MS_Helper_Html::html_element(
				array(
					'id' => 'setup_payment',
					'type' => MS_Helper_Html::TYPE_HTML_LINK,
					'value' => __( 'Zahlungsmöglichkeiten', 'membership2' ),
					'url' => $payment_url,
					'class' => 'wpmui-field-button button',
				)
			);
				...
				<div class="ms-protection-edit-wrapper">
					...
					MS_Helper_Html::html_element(
						array(
							...
							'type' => MS_Helper_Html::TYPE_HTML_LINK,
							'title' => $title,
							'value' => sprintf( __( 'Bearbeite %s Zugriff', 'membership2' ), $title ),
							'url' => $edit_url,
							'class' => 'wpmui-field-button button',
						)
					);
