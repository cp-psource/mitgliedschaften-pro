...
class MS_View_Adminbar extends MS_View {
		...
		<div class="ms-sim-info <?php echo esc_attr( $toggle_state ); ?>">
			<div class="ms-sim-block">
				<h4 class="toggle-wrap">
					<?php _e( 'Simulationsübersicht', 'membership2' ); ?>
				...
				<div class="ms-sim-body">
					...
					<table cellspacing="0" cellpadding="0" width="100%" border="0" class="inside">
						...
						<tr>
							<th><?php _e( 'Zahlungsmodel', 'membership2' ); ?></th>
							<td><?php echo esc_html( $pay_types[ $mem->payment_type ] ); ?></td>
						</tr>
						<tr>
							<th><?php _e( 'Zahlungsdetails', 'membership2' ); ?></th>
	...
	public function prepare_fields() {
		...
		foreach ( $memberships as $membership ) {
			if ( $base_id == $membership->id ) {
				$label = __( '- Keine Mitgliedschaft/Besucher -', 'membership2' );
			} else {
				$label = $membership->name;
				if ( ! $membership->active ) {
					$label .= ' ' . __( '(Inaktiv)', 'membership2' );
				}
			}
		...
		$fields = array(
			'exit_button' => array(
				...
				'value' => __( 'Testmodus beenden', 'membership2' ),
	...
	protected function output_deciding_info( $denied_url, $membership_id, $rules ) {
		...
		if ( ! empty( $denied_url ) ) { ?>
			...
			<?php
			printf(
				__( 'Zugriff verweigert durch %s', 'membership2' ),
			...
			foreach ( $rules as $rule_type ) {
				printf(
					...
					__( 'Durch Regel verweigert:', 'membership2' )
				);
			}
			?>
		<?php } elseif ( $membership->is_valid() ) { ?>
			...
			<?php
			printf(
				__( 'Zugang gewährt von %s', 'membership2' ),
				'<b>' . esc_html( $membership->name ) . '</b>'
			);
			?>
			</div>
			...
			foreach ( $rules as $rule_type ) {
				printf(
					...
					$rule_title[$rule_type],
					__( 'Erlaubt durch Regel:', 'membership2' )
				);
			}
			?>
			</ul>
		<?php } else { ?>
			...
			<?php _e( 'Ungeschützt', 'membership2' ); ?>
			</div>
		<?php
		}
	}
