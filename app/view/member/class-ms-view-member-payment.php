...
class MS_View_Member_Payment extends MS_Dialog {
	...
	public function prepare() {
		...
		// Dialog Title
		$this->title = sprintf(
			__( 'Abonnementdetails: %1$s', 'membership2' ),
	...
	public function get_contents( $data ) {
		...
		foreach ( $subscription->get_payments() as $payment ) {
			...
			$pay_details[] = array(
				'title' => __( 'Aufgezeichnete Zahlung', 'membership2' ),
				'type' => MS_Helper_Html::TYPE_HTML_TABLE,
				'value' => array(
					0 => array( 'Zahlungsdatum', $payment['date'] ),
					1 => array( 'Zahlungs-Gateways', $gateway ),
					2 => array( 'Betrag', $payment['amount'] ),
					3 => array( 'Externe ID', $payment['external_id'] ),
				),
				'field_options' => array(
					'head_col' => true,
				),
		...
		foreach ( $invoices as $invoice ) {
			...
			$transaction_log = sprintf(
				...
				__( 'Transaktion anzeigen', 'membership2' )
			);
			$invoice_list[] = $invoice->id;

			$inv_details[$invoice->id] = array(
				'title' => sprintf( __( 'Rechnung %s', 'membership2' ), $invoice->id ),
				'type' => MS_Helper_Html::TYPE_HTML_TABLE,
				'value' => array(
					0 => array( 'Rechnungs-ID', $invoice->id . $transaction_log ),
					2 => array( 'Erstellungsdatum', $invoice->invoice_date ),
					1 => array( 'Zahlungs-Gateways', $gateway ),
					2 => array( 'Fälligkeitstermin', $invoice->due_date ),
					3 => array( 'Regelmäßige Menge', $invoice->amount ),
					4 => array( 'Insgesamt in Rechnung gestellt', $invoice->total ),
					5 => array( 'Status', $invoice->status ),
					6 => array( 'Anmerkungen', $invoice->description ),
					7 => array( 'Checkout IP', $invoice->checkout_ip ),
					8 => array( 'Checkout Datum', $invoice->checkout_date ),
				),
				'field_options' => array(
					'head_col' => true,
				),
		...
		$transaction_log = array(
			...
			'value' => __( 'Alle Transaktionen für dieses Abonnement anzeigen', 'membership2' ),
			'url' => $transaction_url,
			'target' => '_blank',
		);
