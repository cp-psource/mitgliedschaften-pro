...
class MS_View_Member_Subscription extends MS_Dialog {
	...
	public function get_contents( $data ) {
		...
		if ( isset( $gateways[ $subscription->gateway_id ] ) ) {
			...
			$gateway = __( '- Kein Gateway -', 'membership2' );
		} else {
		...
		$sub_details = array(
			'title' => __( 'Abonnementdetails', 'membership2' ),
			'type' => MS_Helper_Html::TYPE_HTML_TABLE,
			'value' => array(
				array( 'Abonnement-ID', $subscription->id ),
				array( 'Mitgliedschaft', $subscription->get_membership()->name ),
				array( 'Zahlungs-Gateways', $gateway ),
				array( 'Zahlungsart', $subscription->get_payment_description( null, true ) ),
				array( 'Abonnement beginnt', $subscription->start_date ),
				array( 'Abonnement endet', $subscription->expire_date ),
		...
		foreach ( $events as $event ) {
			$evt_details[] = array(
				'title' => __( 'Ereignisdetails', 'membership2' ),
				'type' => MS_Helper_Html::TYPE_HTML_TABLE,
				'value' => array(
					array( 'Ereignis ID', $event->id ),
					array( 'Datum', $event->date ),
					array( 'Beschreibung', $event->description ),
				),
				'field_options' => array(
					'head_col' => true,
				),
