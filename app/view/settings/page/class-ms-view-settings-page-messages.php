...
class MS_View_Settings_Page_Messages extends MS_View_Settings_Edit {
	...
	public function to_html() {
		...
		if ( $membership instanceof MS_Model_Membership ) {
			...
			$desc = sprintf(
				__( 'Hier können die %sStandardeinstellungen%s für diese Mitgliedschaft überschrieben werden.', 'membership2' ),
				'<a href="' . $settings_url . '">',
				'</a>'
			);
		} else {
		...
		<form class="ms-form" action="" method="post">
			...
			$subtitle = apply_filters(
				'ms_translation_flag',
				__( 'Inhalt-Schutznachricht', 'membership2' ),
				'message-protected'
			);
			...
			$subtitle = apply_filters(
				'ms_translation_flag',
				__( 'Shortcode-Schutznachricht', 'membership2' ),
				'message-shortcode'
			);
			MS_Helper_Html::settings_box(
				$fields['shortcode'],
			...
			if ( $has_more ) {
				$subtitle = apply_filters(
					'ms_translation_flag',
					__( 'Weiterlesen Tag-Schutznachricht', 'membership2' ),
					'message-more_tag'
				);
				MS_Helper_Html::settings_box(
					$fields['more_tag'],
	...
	protected function get_fields() {
		...
		$groups['content'] = array(
			'description' => array(
				...
				'value' => __( 'Nachricht wird angezeigt, wenn die gesamte Seite über den Shortcode <code>[ms-protection-message]</code> geschützt ist.', 'membership2' ),
			...
			'override' => array(
				...
				'before' => __( 'Standardnachricht verwenden', 'membership2' ),
				'after' => __( 'Eigene Nachricht definieren', 'membership2' ),
		...
		$groups['shortcode'] = array(
			'description' => array(
				'type' => MS_Helper_Html::TYPE_HTML_TEXT,
				'value' => __( 'Meldung anstelle einer geschützten Shortcode-Ausgabe.', 'membership2' ),
				'wrapper_class' => 'ms-block ms-tcenter',
			),
			'override' => array(
		...
		$groups['more_tag'] = array(
			'description' => array(
				'type' => MS_Helper_Html::TYPE_HTML_TEXT,
				'value' => __( 'Meldung anstelle des Weiterlesen-Inhalts.', 'membership2' ),
				'wrapper_class' => 'ms-block ms-tcenter',
			),
			'override' => array(
				'id' => 'override_content',
