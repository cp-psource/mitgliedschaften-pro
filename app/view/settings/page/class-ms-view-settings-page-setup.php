...
class MS_View_Settings_Page_Setup extends MS_View {
	...
	public function html_short_form() {
		$code = sprintf(
			'<center>%1$s</center>',
			sprintf(
				__( 'Du kannst jetzt auf Seite %sSchutzregeln%s gehen, um Zugriffsebenen für diese Mitgliedschaft einzurichten.', 'membership2' ),
		...
		<div class="ms-setup-form">
			...
			<div class="ms-setup-nav">
				<div class="ms-title">
					...
					<?php _e( 'Bitte wähle Seiten aus, die in Deiner Navigation angezeigt werden sollen', 'membership2' ); ?>
				</div>
				<div class="ms-description">
					<?php
					printf(
						__( 'Du kannst diese später jederzeit ändern, indem Du in Deiner Admin-Seitenleiste auf %1$s gehst.', 'membership2' ),
						sprintf(
							'<a href="%1$s" target="_blank">%2$s</a>',
							admin_url( 'nav-menus.php' ),
							__( 'Darstellung', 'membership2' ) . ' &raquo; ' . __( 'Menüs', 'membership2' )
						)
					);
					?>
				</div>
			...
			<div class="ms-setup-site">
				<div class="ms-title">
					...
					<?php _e( 'Wähle die Seite aus, auf der sich die Seiten für Mitgliedschaften befinden', 'membership2' ); ?>
				</div>
				<div class="ms-description">
					<?php _e( 'Wenn Du die Seite änderst, werden auf der ausgewählten Seite neue PS-Mitgliedschafts-Seiten erstellt. Du kannst diese Seiten jederzeit anpassen oder ersetzen.', 'membership2' ); ?>
				</div>
				<?php
				...
				$site_fields = array(
					array(
						...
						'id' => 'network_site',
						'title' => __( 'Wähle die Seite aus, auf der sich die Seiten für Mitgliedschaften befinden', 'membership2' ),
			...
			<div class="ms-setup-pages">
				<div class="ms-title">
					...
					<?php _e( 'Mitgliedschaften Seiten', 'membership2' ); ?>
				</div>
				<div class="ms-description">
					<?php _e( 'Einrichten der Mitgliedschaften-Seiten, die auf Deiner Website angezeigt werden.', 'membership2' ); ?>
				</div>
				<?php
								...
								<div class="ms-action">
									...
									MS_Helper_Html::html_link(
										array(
											...
											'url' => '',
											'value' => __( 'Seite anzeigen', 'membership2' ),
											'target' => '_blank',
											'data_ms' => array(
									...
									MS_Helper_Html::html_link(
										array(
											...
											'url' => '',
											'value' => __( 'Seite bearbeiten', 'membership2' ),
											'target' => '_blank',
											'data_ms' => array(
												'base' => get_admin_url(
	...
	protected function prepare_fields() {
		...
		foreach ( $page_types as $type => $label ) {
			...
			$pages[ $type ] = array(
				...
				'value' => $page_id,
				'field_options' => array(
					'no_item' => __( '- Wähle eine Seite aus -', 'membership2' ),
				),
				'ajax_data' => array(
					'field' => $type,
					'action' => $pages_action,
	...
	public function show_menu_controls() {
		...
		if ( $can_create_nav ) {
			...
			$button = array(
				'id' => 'create_menu',
				'type' => MS_Helper_Html::INPUT_TYPE_BUTTON,
				'value' => __( 'Okay, erstelle das Menü', 'membership2' ),
				'ajax_data' => array(
			...
			$code = sprintf(
				...
				__( 'Warte! Du hast noch kein Menü erstellt ... <br> Lasse es uns jetzt erstellen, damit Du auswählen kannst, welche Seiten Deinen Besuchern angezeigt werden sollen!', 'membership2' ),
