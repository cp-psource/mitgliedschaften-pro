...
class MS_View_Settings_Page_Communications extends MS_View_Settings_Edit {
	...
	public function to_html() {
		...
		if ( $membership instanceof MS_Model_Membership ) {
			...
			$desc = sprintf(
				__( 'Hier kannst Du %sStandard-Nachrichten%s für diese Mitgliedschaft überschreiben.', 'membership2' ),
				'<a href="' . $settings_url . '">',
				'</a>'
			);
		} else {
	...
	public function wp_footer() {
		 ...
		 */
		$var_button = array(
			'title' => __( 'Füge Mitgliedschaftsvariablen ein', 'membership2' ),
			'items' => $vars,
		);
		...
		printf(
			...
			json_encode(
				__( 'Du hast Änderungen vorgenommen, die noch nicht gespeichert sind. Möchtest Du diese Änderungen verwerfen?', 'membership2' )
			)
		);
	}
	...
	protected function get_fields() {
		...
		$key_active = __( 'Email senden', 'membership2' );
		$key_inactive = __( 'Keine Email', 'membership2' );
		$key_skip = __( 'Standardvorlage verwenden', 'membership2' );
		$titles = array(
			$key_active => array(),
			$key_inactive => array(),
			$key_skip => array(),
		...
		$fields = array(
			...
			'switch_comm_type' => array(
				...
				'value' => __( 'Vorlage laden', 'membership2' ),
			),
			...
			'override' => array(
				...
				'value' => $comm->override,
				'before' => __( 'Standardvorlage verwenden', 'membership2' ),
				'after' => __( 'Benutzerdefinierte Vorlage definieren', 'membership2' ),
			...
			'period_unit' => array(
				...
				'title' => __( 'Zeitraum nach/vor', 'membership2' ),
			...
			'subject' => array(
				...
				'title' => apply_filters(
					'ms_translation_flag',
					__( 'Betreff der Nachricht', 'membership2' ),
			...
			'cc_enabled' => array(
				...
				'title' => __( 'Kopie an Administrator senden', 'membership2' ),
