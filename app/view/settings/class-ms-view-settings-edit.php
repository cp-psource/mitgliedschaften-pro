	...
	public function to_html() {
		...
		<div class="ms-wrap wrap">
			...
			MS_Helper_Html::settings_header(
				array(
					'title' => __( 'Mitgliedschaften Einstellungen', 'membership2' ),
	...
	protected function advanced_forms( $desc ) {
		...
		if ( ! empty( $_GET['reset'] ) ) {
			...
			$desc[] = sprintf(
				...
				'Achtung: Dadurch werden alle Einstellungen und Details zu Mitgliedschaften vollständig gelöscht!',
				sprintf(
					'<form method="POST" action="%s" style="padding:20px 0">' .
					'<label style="line-height:28px">' .
					'<input type="checkbox" name="confirm" value="yes" /> Ja, alles zurücksetzen!' .
					'</label><p>' .
					'<button class="button-primary">Mach es!</button> ' .
					'<a href="%s" class="button">Abbrechen</a>' .
					'</p></form>',
					$reset_url,
					$cancel_url
				)
		...
		if ( ! empty( $_GET['restore'] ) ) {
			...
			$desc[] = sprintf(
				'<div class="error" style="width:600px;margin:20px auto;text-align:center"><p><b>%1$s</b></p><hr />%2$s</div>',
				'Achtung: Dadurch werden vorhandene Daten überschrieben und durch alte Daten aus dem Snapshot ersetzt!',
				sprintf(
					'<form method="POST" action="%s" style="padding:20px 0">' .
					...
					'<input type="checkbox" name="confirm" value="yes" /> Ja, Daten überschreiben!' .
					'</label><p>' .
					'<button class="button-primary">Mach es!</button> ' .
					'<a href="%s" class="button">Abbrechen</a>' .
					'</p></form>',
        ...
        if ( ! empty( $_GET['fixsub'] ) ) {
            ...
            $desc[] = sprintf(
                '<div class="error" style="width:600px;margin:20px auto;text-align:center"><p><b>%1$s</b></p><hr />%2$s</div>',
                'Achtung: Dies kann den Abonnementstatus einiger Mitglieder ändern!',
                sprintf(
                    '<form method="POST" action="%s" style="padding:20px 0">' .
                    '<label style="line-height:28px">' .
                    '<input type="checkbox" name="confirm" value="yes" /> Ja, Abonnements reparieren!' .
                    '</label><p>' .
                    '<button class="button-primary">Mach es!</button> ' .
                    '<a href="%s" class="button">Abbrechen</a>' .
                    '</p></form>',
	...
	protected function render_settings_footer( $tab_name , $show = true ) {
		...
		if ( $status_stamp > 0 ) {
			$status_delay = sprintf(
				__( 'in %s Stunden %s Minuten', 'membership2' ),
				floor( ($status_stamp - 1) / 3600 ),
				date( 'i', $status_stamp )
			);
		} else {
			$status_delay = __( '(jetzt...)', 'membership2' );
		}
		...
		$lbl_run = __( 'Jetzt durchführen!', 'membership2' );
		...
		if ( MS_Plugin::get_modifier( 'MS_LOCK_SUBSCRIPTIONS' ) ) {
			_e( 'Mitgliedschaftsstatusprüfungen sind deaktiviert.', 'membership2' );
			echo ' ';
		} else {
			printf(
				__( 'Überprüfe die Änderungen des Mitgliedschaftsstatus %s.', 'membership2' ) . ' ',
				'<a href="' . $status_url . '" title="' . $lbl_run . '">' . $status_delay . '</a>'
			);
		}

		if ( MS_Plugin::get_modifier( 'MS_STOP_EMAILS' ) ) {
			_e( 'Das Senden von E-Mail-Antworten ist deaktiviert.', 'membership2' );
		} else {
			...
			if ( $email_stamp > 0 ) {
				...
				$email_delay = __( '(jetzt...)', 'membership2' );
			}
			...
			if ( $show ) {
				...
				if ( ! $count ) {
					$msg = __( 'Keine ausstehenden E-Mail-Antworten gefunden', 'membership2' );
				} elseif ( 1 == $count ) {
					$msg = __( 'Eine ausstehende E-Mail-Antwort senden %1$s', 'membership2' );
				} else {
					$msg = __( 'Sende %2$s ausstehende E-Mail-Antworten %1$s', 'membership2' );
				}
