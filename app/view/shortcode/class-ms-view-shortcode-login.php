...
class MS_View_Shortcode_Login extends MS_View {
	...
	public function to_html() {
		...
		if ( 'logout' === $form ) {
			...
			if ( $ajax_login ) {
				...
				$res_form .= '<br/><a href="'.wp_lostpassword_url( get_bloginfo('url') ).'" title="'.__( 'Passwort vergessen', 'membership2' ).'">'.__( 'Passwort vergessen', 'membership2' ).'</a>';
			}
	...
	private function login_header_html() {
		...
		<div class="ms-alert-box ms-alert-error ms-auth-header">
			<?php _e( 'Bitte melde Dich an, um auf diese Seite zuzugreifen.', 'membership2' ); ?>
		</div>
		<?php endif;
	...
	private function login_form( $redirect_to = null ) {
		...
		$defaults = array(
			...
			'label_remember' 	=> __( 'Behalte mich in Erinnerung', 'membership2' ),
		...
		<form
				...
				<?php if ( 'top' === $nav_pos ) : ?>
					<div class="nav">
						<p><a class="lost" href="#lostpassword">
							<?php _e( 'Passwort vergessen?', 'membership2' ); ?>
						</a></p>
					</div>
				<?php endif; ?>
			...
			<div class="nav">
				<p><a class="lost" href="#lostpassword"><?php _e( 'Passwort vergessen?', 'membership2' ); ?></a></p>
			<?php endif; ?>
			</div>
		</form>
		<?php
	...
	private function lostpass_form() {
		...
		<form
			...
			<div class="form">
				...
				<?php if ( 'top' === $nav_pos ) : ?>
					<div class="nav">
						<p><a class="login" href="#login"><?php _e( 'Einloggen', 'membership2' ); ?></a></p>
					</div>
				<?php endif; ?>
				<?php
				/**
	...
	private function logout_form() {
		...
		$yourname = sprintf(
			__( 'Du bist eingeloggt als %s.', 'membership2' ),
		...
		if ( null === $Reset_Result ) {
			...
			if ( ! $user || is_wp_error( $user ) ) {
				...
				if ( $user && 'expired_key' == $user->get_error_code() ) {
					...
					$err_msg->add(
						...
						__( 'Entschuldigung, dieser Reset-Schlüssel ist nicht mehr gültig. Bitte fordere eine neue Reset-E-Mail an und versuche es erneut.', 'membership2' )
					);
				} else {
					$fatal_error = true;
					$err_msg->add(
						'password_invalid_key',
						__( 'Entschuldigung, wir haben keinen gültigen Reset-Schlüssel gefunden. Bitte fordere eine neue Reset-E-Mail an und versuche es erneut.', 'membership2' )
					);
				}
			} else {
				...
				if ( $pass1 && $pass1 != $pass2 ) {
					...
					$err_msg->add(
						...
						__( 'Die Passwörter stimmen nicht überein. Versuche es erneut.', 'membership2' )
					);
				}
			}
			...
			if ( $fatal_error && count( $err_msg->errors ) ) {
				...
				$Reset_Result = sprintf(
					...
					$url,
					__( 'Fordere einen neuen Schlüssel zum Zurücksetzen des Passworts an', 'membership2' )
				);
			} elseif ( $pass1 ) {
				...
				$Reset_Result = sprintf(
					...
					__( 'Dein Passwort wurde zurück gesetzt.', 'membership2' ),
					$url,
					__( 'Melde Dich mit Deinem neuen Passwort an', 'membership2' )
				);
			} else {
				// This action is documented in wp-login.php
				do_action( 'validate_password_reset', $err_msg, $user );
				...
				<form name="resetpassform" id="resetpassform"
					...
					<p class="user-pass1-wrap">
						<label for="pass1"><?php _e( 'Neues Passwort', 'membership2' ) ?></label><br />
						<div class="wp-pwd">
							...
							<div id="pass-strength-result" class="hide-if-no-js" aria-live="polite"><?php _e( 'Passwortstärke', 'membership2' ); ?></div>
						</div>
					</p>
					<p class="user-pass2-wrap">
						<label for="pass2"><?php _e( 'Bestätige neues Passwort', 'membership2' ) ?></label><br />
	...
	public function enqueue_scripts() {
		...
		mslib3()->ui->data(
			'ms_ajax_login',
			array(
				'loginmessage' 		=> __( 'Bitte melde Dich an, um auf diese Seite zuzugreifen.', 'membership2' ),
				'resetmessage' 		=> __( 'Bitte gib Deine Daten ein, um Dein Passwort zurückzusetzen', 'membership2' ),
				'ajaxurl' 			=> admin_url( 'admin-ajax.php', is_ssl() ? 'https' : 'http' ),
				'loadingmessage' 	=> __( 'Bitte warten...', 'membership2' ),
				'errormessage' 		=> __( 'Anfrage fehlgeschlagen, bitte versuche es erneut.', 'membership2' ),
			)
		);
	}
