...
class MS_Gateway_Stripe extends MS_Gateway {
	...
	public function process_purchase( $subscription ) {
		...
		if ( ! empty( $_POST['stripeToken'] ) ) {
			...
			try {
				...
				$note = 'Stripe Fehler: '. $e->getMessage();
	...
	public function request_payment( $subscription ) {
		...
		if ( ! $invoice->is_paid() ) {
			try {
				...
				if ( ! empty( $customer ) ) {
					if ( 0 == $invoice->total ) {
						$invoice->changed();
						$success 	= true;
						$note 		= __( 'Keine Zahlung für kostenlose Mitgliedschaft', 'membership2' );
					} else {
						$charge = $this->_api->charge(
							$customer,
							$invoice->total,
					...
					$note = "Der Stripekunde ist leer für den Benutzer $member->username";
					MS_Helper_Debug::debug_log( $note );
				}
			} catch ( Exception $e ) {
				$note = 'Stripe Fehler: '. $e->getMessage();
			...
			$was_paid 	= true;
			$note 		= __( 'Rechnung bereits bezahlt', 'membership2' );
		}
