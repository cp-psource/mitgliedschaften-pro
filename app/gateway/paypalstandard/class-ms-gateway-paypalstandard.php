	...
	public function after_load() {
		...
		if ( $this->active && $this->is_live_mode() && strpos( $this->merchant_id, '@' ) ) {
			...
			mslib3()->ui->admin_message(
				sprintf(
					__( 'Warnung: Du verwendest Deine E-Mail-Adresse für das PayPal Standard-Gateway anstelle Deiner Händler-ID. Bitte überprüfe %sDeine Zahlungseinstellungen%s und gib stattdessen die Händler-ID ein', 'membership2' ),
					'<a href="' . $settings_url . '">',
					'</a>'
				),
				'err'
	...
	public function handle_return( $log = false ) {
				...
				if ( ! empty( $_POST['custom'] ) ) {
						...
						if ( 'm1' == $ext_type ) {
							...
							if ( $subscription ) {
								...
								if ( $user && $user->ID == $m1_user_id ) {
									...
									if ( $membership ) {
										$notes_err = sprintf(
											'Benutzer hat die Mitgliedschaft %s nicht abonniert.',
											$membership->id
										);
									} else {
										$notes_err = 'Eine Mitgliedschaft konnte nicht festgestellt werden.';
									}
								} else {
									$notes_err = sprintf(
										'Benutzer mit ID %s konnte nicht gefunden werden.',
										$m1_user_id
									);
					...
					if ( $user && $user->ID ) {
						...
						if ( $subscription ) {
							...
							if ( $membership ) {
								$notes_err = sprintf(
									'Der Benutzer hat die Mitgliedschaft %s nicht abonniert.',
									$membership->id
								);
							} else {
								$notes_err = 'Eine Mitgliedschaft konnte nicht festgestellt werden.';
		...
		if ( $transaction_type ) {
			switch ( $transaction_type ) {
				...
				case 'subscr_signup':
					...
					$notes_txn = __( 'Abonnement wurde erstellt', 'membership2' );
					$do_action = 'pay-free';
					break;

				case 'subscr_payment':
					// Payment was received
					$notes_txn = __( 'Bezahlung erfolgreich', 'membership2' );
					$do_action = 'pay-free';
					break;

				case 'recurring_payment':
					// Payment was received
					$notes_txn = __( 'Wiederkehrende Zahlung erfolgreich', 'membership2' );
					$do_action = 'pay-free';
					break;

				case 'subscr_modify':
					// Payment profile was modified
					$notes_txn = __( 'Das Abonnement wurde geändert', 'membership2' );
					break;

				case 'subscr_failed':
					// Payment profile was modified
					$notes_txn = __( 'Bezahlung fehlgeschlagen', 'membership2' );
					$do_action = 'error';
					break;
				...
				case 'subscr_cancel':
					...
					$notes_txn 		= __( 'Das Abonnement wurde gekündigt', 'membership2' );
					$do_action 		= 'cancel';
				...
				case 'recurring_payment_suspended':
					...
					$notes_txn 		= __( 'Das Abonnement wurde ausgesetzt', 'membership2' );
					$do_action 		= 'cancel';
					$payment_status = false; // Skip the payment_status check.
					break;
				...
				case 'recurring_payment_suspended_due_to_max_failed_payment':
					...
					$notes_txn 		= __( 'Abonnement ist fehlgeschlagen', 'membership2' );
					$do_action 		= 'cancel';
					$payment_status = false; // Skip the payment_status check.
					break;
				...
				case 'new_case':
					...
					if ( ! empty( $_POST['buyer_additional_information'] ) ) {
						$notes_txn = sprintf(
							__( 'Neuer Streit eingereicht: %s', 'membership2' ),
							$_POST['buyer_additional_information']
						);
					} else {
						$notes_txn = __( 'Neuer Streit eingereicht', 'membership2' );
					}
				...
				case 'subscr_eot':
					 ...
					 */
					$notes_txn = __( 'Abonnement beendet', 'membership2' );
					break;

				case 'recurring_payment_profile_created':
					$notes_txn = __( 'Wiederkehrendes Zahlungsprofil erstellt', 'membership2' );
					break;

				case 'recurring_payment_failed':
					$notes_txn = __( 'Wiederkehrende Zahlung fehlgeschlagen', 'membership2' );
					$do_action = 'error';
					break;

				case 'recurring_payment_profile_cancel':
					$notes_txn = __( 'Wiederkehrendes Zahlungsprofil storniert', 'membership2' );
					break;

				case 'recurring_payment_expired':
					$notes_txn = __( 'Das Profil für wiederkehrende Zahlungen ist abgelaufen', 'membership2' );
					break;

				case 'recurring_payment_skipped':
					$notes_txn = __( 'Wiederkehrendes Zahlungsprofil übersprungen', 'membership2' );
					break;
				...
				case 'recurring_payment_outstanding_payment':
					...
					if ( is_numeric( $os_amount) && floatval( $os_amount ) ) {
						$notes_txn = sprintf(
							__( 'Ausstehende Zahlung von %s', 'membership2' ),
							trim( $currency . ' ' . $os_amount )
						);
					} else {
						$notes_txn = __( 'Ausstehende Zahlung', 'membership2' );
					}
					break;
				...
				case 'recurring_payment_outstanding_payment_failed':
					...
					if ( is_numeric( $os_amount) && floatval( $os_amount ) ) {
						$notes_txn = sprintf(
							__( 'Das Sammeln der Zahlung von %s ist fehlgeschlagen', 'membership2' ),
							trim( $currency . ' ' . $os_amount )
						);
					} else {
						$notes_txn = __( 'Zahlungseingang fehlgeschlagen', 'membership2' );
					}
					$do_action = 'error';
					break;
				...
				case 'send_money':
					if ( ! empty( $_POST['memo'] ) ) {
						$notes_txn = sprintf(
							__( 'Geld gesendet für: %s', 'membership2' ),
							$_POST['memo']
						);
					} else {
						$notes_txn = __( 'Geld gesendet', 'membership2' );
					}
					break;
				...
				default:
					...
					$notes_txn = sprintf(
						__( 'txn_type wird nicht behandelt: %s', 'membership2' ),
						$transaction_type
					);
					break;
			}
		...
		if ( $payment_status ) {
			switch ( $payment_status ) {
				...
				case 'reversed':
					$notes_pay 	= __( 'Letzte Transaktion wurde storniert: Zahlung wurde storniert (Rückbuchung).', 'membership2' );
					$status 	= MS_Model_Invoice::STATUS_DENIED;
					break;

				case 'refunded':
					$notes_pay 	= __( 'Letzte Transaktion wurde storniert: Zahlung wurde zurückerstattet.', 'membership2' );
					$status 	= MS_Model_Invoice::STATUS_DENIED;
					break;

				case 'denied':
					$notes_pay 	= __( 'Letzte Transaktion wurde storniert: Zahlung abgelehnt.', 'membership2' );
					$status 	= MS_Model_Invoice::STATUS_DENIED;
					$do_action 	= 'error';
					break;
				...
				case 'pending':
					...
					$notes_pay = __( 'Letzte Transaktion steht noch aus.', 'membership2' ) . ' ';

					switch ( $_POST['pending_reason'] ) {
						case 'address':
							$notes_pay .= __( 'Keine bestätigte Lieferadresse', 'membership2' );
							break;

						case 'authorization':
							$notes_pay .= __( 'Mittel noch nicht erfasst', 'membership2' );
							break;

						case 'echeck':
							$notes_pay .= __( 'eCheck wurde noch nicht gelöscht', 'membership2' );
							break;

						case 'intl':
							$notes_pay .= __( 'Bis zur Genehmigung durch den Dienstleister', 'membership2' );
							break;

						case 'multi-currency':
							$notes_pay .= __( 'Ausstehender Prozess mit mehreren Währungen durch den Dienstanbieter', 'membership2' );
							break;

						case 'unilateral':
							$notes_pay .= __( 'Keine bestätigte E-Mail-Adresse', 'membership2' );
							break;

						case 'upgrade':
							$notes_pay .= __( 'Bis zum Upgrade des PayPal-Kontos', 'membership2' );
							break;

						case 'verify':
							$notes_pay .= __( 'Ausstehende Überprüfung des PayPal-Kontos', 'membership2' );
							break;

						default:
							$notes_pay .= __( 'Unbekannter Grund', 'membership2' );
							break;
					}
				...
				case 'in-progress':
					$notes_pay = sprintf(
						__( 'Zahlungsstatus nicht verarbeitet: %s', 'membership2' ),
						$payment_status
					);
					break;
			}
		...
		if ( $transaction_exists ) {
			$notes = 'Duplikat: Diese Transaktion wurde bereits verarbeitet.';
			$success = false;
			$ignore = true;
		}
		...
		elseif ( $invoice_id ) {
			...
			if ( ! is_wp_error( $response )
				...
				switch ( $do_action ) {
					case 'pay':
						...
						if ( $amount == $invoice->total ) {
							$notes_pay .= __( 'Bezahlung erfolgreich', 'membership2' );
						} else {
							$notes_pay .= __( 'Zahlung registriert, obwohl der Betrag von der Rechnung abweicht.', 'membership2' );
						}
						break;
				...
				switch ( true ) {
					case is_wp_error( $response ):
						$reason = 'PayPal hat diese Transaktion nicht überprüft: Unbekannter Fehler';
						break;

					case 200 != $response['response']['code']:
						$reason = sprintf(
							'PayPal hat die Transaktion nicht überprüft: Code %s',
							$response['response']['code']
						);
						break;

					case empty( $response['body'] ):
						$reason = 'PayPal hat diese Transaktion nicht überprüft: Leere Antwort';
						break;

					case 'VERIFIED' != $response['body']:
						$reason = sprintf(
							'PayPal hat diese Transaktion nicht überprüft: "%s"',
							$response['body']
						);
						break;
					...
					case ! $invoice->id:
						$reason = sprintf(
							'Angegebene Rechnung existiert nicht: "%s"',
							$invoice_id
						);
						break;
				}
			...
			if ( 'm1' == $ext_type ) {
				 ...
				 */
				$notes 		= 'M1 Zahlung erkannt. Manueller Abgleich erforderlich. ' . $notes_err;
				$success 	= false;
				 ...
				 */
				$notes 		= 'PayPal-Zahlungsschaltfläche erkannt. Manuelle Anpassung erforderlich. ' . $notes_err;
				$success 	= false;
			} else {
				...
				if ( ! empty( $notes_err ) ) {
					...
					$notes = 'Ignoriert: txn_type nicht angegeben. Kann nicht verarbeiten.';
				} elseif ( empty( $_POST['invoice'] ) && empty( $_POST['custom'] ) ) {
					$notes = 'Ignoriert: Keine Rechnung oder benutzerdefinierten Daten angegeben.';
				} else {
					$notes = 'Ignoriert: Fehlende POST-Variablen. Eine Identifizierung ist nicht möglich.';
				}
