...
class MS_Gateway_Authorize extends MS_Gateway {
	...
	public function process_purchase( $subscription ) {
		...
		if ( ! is_ssl() ) {
			throw new Exception( __( 'Du musst dazu HTTPS verwenden', 'membership2' ) );
		}
		...
		if ( empty( $cim_profile_id ) ) {
			...
			if ( $response->isError() ) {
				throw new Exception(
					__( 'Das ausgewählte Zahlungsprofil ist ungültig, gib eine neue Kreditkarte ein', 'membership2' )
				);
			}
		} else {
	...
	protected function online_purchase( &$invoice, $member, $log_action ) {
		...
		if ( 0 == $invoice->total ) {
			$notes = __( 'Insgesamt ist Null. Zahlung genehmigt. Nicht an das Gateway gesendet.', 'membership2' );
			...
			if ( $response->isOk() ) {
				...
				if ( $transaction_response->approved ) {
					...
					$notes = sprintf(
						__( 'Zahlung fehlgeschlagen: Code %s, Subcode %s, Ursachencode %s, Grund %s', 'membership2' ),
				...
				$notes = __( 'Bezahlung fehlgeschlagen: ', 'membership2' ) . $response->getMessageText();
			}
		} elseif ( $need_code && ! $have_code ) {
			$notes = __( 'Sichere Zahlung fehlgeschlagen: Kein Kartencode gefunden', 'membership2' );
		}
	...
	protected function create_cim_profile( $member ) {
		...
		if ( $response->isError() ) {
			...
			if ( 'E00039' == $response->xml->messages->message->code ) {
				$cim_profile_id = str_replace( 'Ein doppelter Datensatz mit ID ', '', $response->xml->messages->message->text );
				$cim_profile_id = (int) str_replace( ' ist bereits vorhanden.', '', $cim_profile_id );
				...
				throw new Exception(
					__( 'Die Zahlung ist fehlgeschlagen, da das CIM-Profil nicht erstellt wurde: ', 'membership2' ) . $response->getMessageText()
				);
			}
		}
	...
	public function update_cim_profile( $member ) {
		...
		if ( $response->isError() && 'E00039' != $response->xml->messages->message->code ) {
			throw new Exception(
				__( 'Zahlung fehlgeschlagen, da CIM-Profil nicht aktualisiert wurde: ', 'membership2' ) . $response->getMessageText()
			);
		}
	...
	protected function get_cim_transaction( $member ) {
		...
		if ( empty( $cim_profile_id ) || empty( $cim_payment_profile_id ) ) {
			throw new Exception( __( 'CIM Zahlungsprofil nicht gefunden', 'membership2' ) );
		}
