					...
					if ( empty( $_POST['username'] ) || empty( $_POST['password'] ) ) {
						...
						if ( $can_login ) {
							...
							if ( is_wp_error( $user_signon ) ) {
								$resp['error'] = __( 'Benutzername oder Passwort falsch', 'membership2' );
							} else {
								...
								if ( $user_activation_status != 1 ) {
									...
									$resp['error'] = __( 'Konto nicht verifiziert. Bitte überprüfe Deine E-Mail für einen Bestätigungslink', 'membership2' );

								} else {

									$resp['loggedin'] = true;
									$resp['success']  = __( 'Einloggen...', 'membership2' );
